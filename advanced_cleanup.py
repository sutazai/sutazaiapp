...
class AdvancedCleanupManager:
    ...
    def cleanup_timeshift_snapshots(self, keep_recent: int = 3):
        ...
        try:
            ...
            snapshots = subprocess.check_output(snapshots_cmd, shell=True).decode().splitlines()
            
            # Delete older snapshots
            for snapshot in snapshots[keep_recent:]:
                try:
                    subprocess.run(['sudo', 'timeshift', '--delete', '--snapshot', snapshot.split()[0]], check=True)
    ...
    def reduce_swap_and_hibernate(self):
        ...
        try:
            ...
            subprocess.run(['sudo', 'dd', 'if=/dev/zero', 'of=/swapfile', 'bs=1G', 'count=4'], check=True)
            ...
            subprocess.run(['sudo', 'systemctl', 'mask', 'hibernate.target'], check=True)
        ...
        for cache_dir in cache_dirs:
            try:
                if os.path.exists(cache_dir):
                    ...
                    du_output = subprocess.check_output(['du', '-sh', cache_dir]).decode().strip()
                    self.logger.info(f"Cache directory size before cleanup: {cache_dir} - {du_output}")
