...
class DiagramParser:
    ...
    def __init__(self, output_dir: str = "/opt/sutazaiapp/doc_data/diagrams", max_file_size_mb: int = 20):
        """
        Initialize DiagramParser

        Args:
            output_dir (str): Directory to save parsed diagram results
    ...
    def _detect_contours(self, image: np.ndarray) -> List[Dict[str, Any]]:
        """
    ...
    def analyze_diagram(self, file_path: str) -> Dict[str, Any]:
        ...
        Args:
            file_path (str): Path to diagram image
        ...
        try:
            ...
            # Save parsed content
            output_file = os.path.join(self.output_dir, f"{os.path.basename(file_path)}_analysis.json")
            ...
            logger.info(f"Diagram analyzed successfully: {file_path}")
            return result

        except Exception as e:
            logger.exception(f"Diagram analysis error: {e}")
            return {"error": str(e)}
