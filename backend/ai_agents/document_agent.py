...
class DocumentAgent(BaseAgent):
    ...
    def __init__(self, config: AgentConfig, agent_manager=None):
        ...
        self.tesseract_config = config.parameters.get("tesseract_config", {})
        # self.doc_processor = DocumentProcessor() # Removed unused instantiation
    ...
    def _check_dependencies(self) -> None:
        ...
        try:
            # Check pdf2image version safely
            pdf2image_version = importlib.metadata.version('pdf2image')
            logger.info(f"pdf2image version {pdf2image_version} found.")
            self.pdf2image_available = True
        except importlib.metadata.PackageNotFoundError:
            logger.warning("pdf2image not available: Package not found")
            self.pdf2image_available = False
        except Exception as e:
            logger.warning(f"pdf2image not available: {str(e)}") # Catch other potential errors
            self.pdf2image_available = False
    ...
    def _get_version(self) -> str:
        ...
        try:
            # Use importlib.metadata for safe version retrieval
            return importlib.metadata.version("sutazaiapp")
        except importlib.metadata.PackageNotFoundError:
            logger.warning("'sutazaiapp' package not found for version retrieval. Returning default.")
