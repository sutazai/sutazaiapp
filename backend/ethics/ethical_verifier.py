...
class EthicalVerifier:
    ...
    def verify_content(self, content: str) -> Dict[str, Any]:
        ...
        if not self.content_policies.get("allow_profanity", False):
            ...
            profanity_patterns = [
                re.compile(r"(?i)\b(f[*u]ck|sh[*i]t|b[*i]tch|a[*s]shole|c[*u]nt)\b")
            ]
        ...
        if not self.content_policies.get("allow_medical_advice", False):
            ...
            medical_patterns = [
                re.compile(r"(?i)how (to treat|to cure|to heal|do I treat|should I treat)"),
                re.compile(r"(?i)(diagnosis|diagnose) (my|his|her|their)"),
                re.compile(r"(?i)what (medication|drug|prescription|dosage)"),
                re.compile(r"(?i)(stop|discontinue) (taking|using) (medication|drug|prescription)"),
            ]
