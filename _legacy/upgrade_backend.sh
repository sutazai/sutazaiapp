...
check_service "app/intelligent_backend_final.py" "Current Backend"
...
mkdir -p "$BACKUP_DIR"
cp app/intelligent_backend_final.py "$BACKUP_DIR/" 2>/dev/null

# 5. Stop current backend if running
if [ $CURRENT_BACKEND_RUNNING -eq 0 ]; then
    echo -e "\n${YELLOW}5. Stopping current backend${NC}"
    stop_process "app/intelligent_backend_final.py" "app/intelligent_backend_final.py"
    sleep 3
fi
...
if [ ! -f "intelligent_backend_enterprise.py" ]; then
    ...
    nohup python3 app/intelligent_backend_final.py > logs/backend.log 2>&1 &
    exit 1
fi
...
if check_service "intelligent_backend_enterprise.py" "Enhanced Backend"; then
    ...
    if check_service "app/intelligent_backend_final.py" "Original Backend"; then
