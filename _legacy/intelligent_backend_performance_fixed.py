...
try:
    ...
    debug_mode = secure_config.DEBUG
    environment = secure_config.ENVIRONMENT
    JWT_SECRET = secure_config.JWT_SECRET
except Exception as e:
    # Fallback to secure defaults if config fails
    debug_mode = False
    environment = "production"
    JWT_SECRET = "fallback_secret_key_change_me"
    print(f"Warning: Could not load secure config, using defaults: {e}")
...
if environment == "development":
    ...
    allowed_origins = [
        "https://staging.sutazai.local",
        "http://localhost:8501"
    ]
else:  # production
    allowed_origins = [
        "https://sutazai.local",
        "https://app.sutazai.local"
    ]

app.add_middleware(
    CORSMiddleware,
    allow_origins=allowed_origins,
    allow_credentials=False,  # Security: Disable credentials for cross-origin
...
async def detailed_metrics(current_user: str = Depends(verify_token)):
    default_logger.info(f"Detailed metrics accessed by user: {current_user}", category='auth')
