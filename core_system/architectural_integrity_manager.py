...
class ArchitecturalIntegrityManager:
    ...
    def __init__(
        ...
        self.log_dir = log_dir or os.path.join(base_dir, 'logs', 'architectural_integrity')
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(os.path.join(self.log_dir, 'architectural_integrity.log')),
    ...
    def _analyze_project_structure(self) -> Dict[str, Any]:
        ...
        for root, dirs, files in os.walk(self.base_dir):
            ...
            for file in files:
                ...
                structural_analysis['directories'][relative_path]['file_types'][file_ext] = \
                    structural_analysis['directories'][relative_path]['file_types'].get(file_ext, 0) + 1
            ...
            for file in files:
                if file.endswith('.py'):
                    ...
                    try:
                        ...
                        code_quality_metrics['complexity_distribution'][relative_path] = complexity
                        
                        # Documentation coverage
                        doc_coverage = self._calculate_documentation_coverage(tree)
                        code_quality_metrics['documentation_coverage'][relative_path] = doc_coverage
                        
                        # Type hint usage
                        type_hint_usage = self._analyze_type_hint_usage(tree)
                        code_quality_metrics['type_hint_usage'][relative_path] = type_hint_usage
                        
                        code_quality_metrics['total_modules'] += 1
                    
                    except Exception as e:
                        self.logger.warning(f"Code quality analysis failed for {file_path}: {e}")
            ...
            for file in files:
                if file.endswith('.py'):
                    ...
                    try:
                        ...
                        self.logger.warning(f"Design pattern detection failed for {file_path}: {e}")
    ...
    def _detect_architectural_integrity_issues(self) -> List[Dict[str, Any]]:
        ...
        for module, metrics in dependency_report.architectural_insights.get('coupling_metrics', {}).items():
            ...
            for file in files:
                if file.endswith('.py'):
                    ...
                    try:
                        ...
                        cross_reference_map['inheritance_relationships'][relative_path] = [
                            {
                                'class': node.name,
                        ...
                        cross_reference_map['function_calls'][relative_path] = [
                            {
                                'function': node.func.id if isinstance(node.func, ast.Name) else 
                                            f"{node.func.value.id}.{node.func.attr}" if isinstance(node.func, ast.Attribute) else 
                                            'Unknown',
                                'line': node.lineno
                            }
                            for node in ast.walk(tree) 
                        ...
                        self.logger.warning(f"Cross-reference mapping failed for {file_path}: {e}")
    ...
    def _generate_optimization_recommendations(
        ...
        for module, complexity in code_quality_metrics.get('complexity_distribution', {}).items():
    ...
    def visualize_architectural_graph(self, output_path: Optional[str] = None):
        ...
        try:
            ...
            for module, imports in self.cross_reference_map.get('module_imports', {}).items():
...
def main():
    ...
    print(f"Total Modules: {architectural_report.code_quality_metrics.get('total_modules', 0)}")
    
    print("\nArchitectural Patterns:")
    for category, count in architectural_report.architectural_patterns.get('module_categories', {}).items():
