...
class SystemValidator:
    ...
    def check_resource_usage(self) -> Dict[str, float]:
        ...
        if resource_usage["cpu_usage"] > 80:
            self.validation_report["recommendations"].append(
                "High CPU usage: Consider optimizing processes"
            )

        if resource_usage["memory_usage"] > 85:
            self.validation_report["recommendations"].append(
                "High memory usage: Consider adding more RAM or optimizing memory-intensive applications"
            )
    ...
    def run_performance_tests(self) -> Dict[str, float]:
        ...
        # Python startup time
        try:
            startup_time_output = subprocess.check_output(
                [sys.executable, "-m", "timeit", "pass"],
    ...
    def generate_report(self) -> str:
        ...
        try:
            ...
            print(
                f"OS: {self.validation_report['system_info']['os']['name']} {self.validation_report['system_info']['os']['version']}"
            )
            print(
                f"Python: {self.validation_report['system_info']['python']['version']}"
            )
