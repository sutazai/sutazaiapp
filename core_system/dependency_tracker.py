...
class AdvancedDependencyTracker:
                ...
                for file in files:
                    if file.endswith('.py'):
                        ...
                        try:
                            ...
                            module_dependencies = self._analyze_module_dependencies(full_path)
                            
                            # Add edges for dependencies
                            for dep in module_dependencies:
                                if dep != module_name:
                                    self.module_dependency_graph.add_edge(module_name, dep)
                        
                        except Exception as e:
                            self.logger.warning(f"Module dependency analysis failed for {full_path}: {e}")
    ...
    def detect_circular_dependencies(self, dependency_graph: nx.DiGraph) -> List[List[str]]:
        """
        ...
        try:
            ...
            if cycles:
                ...
                for cycle in cycles:
                    self.logger.warning(f"Circular Dependency Cycle: {' -> '.join(cycle)}")
        ...
        try:
            ...
            for pkg, version in compatibility_analysis['version_distribution'].items():
                ...
                conflicting_versions = [
                    other_pkg for other_pkg, other_ver in compatibility_analysis['version_distribution'].items()
    ...
    def generate_comprehensive_dependency_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Comprehensive dependency report generated: {report_path}")
...
def main():
    ...
    try:
        ...
        dependency_report = dependency_tracker.generate_comprehensive_dependency_report()
        
        # Print key insights
        print("Dependency Tracking Insights:")
        print(f"Total Modules: {len(dependency_report.get('module_dependency_graph', {}).get('nodes', []))}")
        print(f"Circular Dependencies: {len(dependency_report.get('circular_dependencies', []))}")
        ...
        for conflict in compatibility.get('potential_conflicts', []):
            print(f"- {conflict['package']}: Conflicting with {conflict['conflicting_packages']}")
