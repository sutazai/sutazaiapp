...
class SystemHealthMonitorWorker:
    ...
    def __init__(
        ...
        self.config_path = config_path or os.path.join(base_dir, 'config', 'system_health_config.yml')
        self.log_dir = log_dir or os.path.join(base_dir, 'logs', 'system_health_monitor')
    ...
    def check_job_status(self, job_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Job status check failed for {job_config.get('name')}: {e}")
            return job_status
