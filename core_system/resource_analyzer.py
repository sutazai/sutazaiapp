...
class ResourceAnalyzer:
    ...
    def get_disk_info(self) -> List[Dict[str, Union[str, int, float]]]:
        ...
        for partition in psutil.disk_partitions():
            try:
                ...
                self.logger.debug(f"Could not check disk {partition.mountpoint}: {str(e)}")
        ...
        if not isinstance(cpu_info, dict) or "error" not in cpu_info:
            if cpu_info.get("percent", 0) > self.cpu_threshold:
                alerts.append({
                    ...
                    "severity": "WARNING" if cpu_info.get("percent", 0) < 95 else "CRITICAL",
                })
        ...
        if not isinstance(memory_info, dict) or "error" not in memory_info:
            if memory_info.get("percent", 0) > self.memory_threshold:
                alerts.append({
                    ...
                    "severity": "WARNING" if memory_info.get("percent", 0) < 95 else "CRITICAL",
                })
        
        # Disk check
        disk_info = self.get_disk_info()
        if isinstance(disk_info, list) and disk_info and "error" not in disk_info[0]:
            for disk in disk_info:
                if disk.get("percent", 0) > self.disk_threshold:
                    alerts.append({
                        ...
                        "severity": "WARNING" if disk.get("percent", 0) < 95 else "CRITICAL",
                    })
        
        # GPU check
        gpu_info = self.get_gpu_info()
        if isinstance(gpu_info, list) and gpu_info and "error" not in gpu_info[0]:
            for gpu in gpu_info:
                if gpu.get("memory_percent", 0) > self.gpu_threshold:
                    alerts.append({
                        ...
                        "severity": "WARNING" if gpu.get("memory_percent", 0) < 95 else "CRITICAL",
                    })
    ...
    def update_history(self) -> None:
        ...
        disk_info = self.get_disk_info()
        if isinstance(disk_info, list) and disk_info and "error" not in disk_info[0]:
            disk_percent = sum(disk.get("percent", 0) for disk in disk_info) / len(disk_info)
        ...
        gpu_info = self.get_gpu_info()
        if isinstance(gpu_info, list) and gpu_info and "error" not in gpu_info[0]:
            gpu_percent = sum(gpu.get("memory_percent", 0) for gpu in gpu_info) / len(gpu_info)
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                if self.history["cpu"] else 0,
                "memory_avg": sum(self.history["memory"]) / len(self.history["memory"]) 
                if self.history["memory"] else 0,
                "disk_avg": sum(self.history["disk"]) / len(self.history["disk"]) 
                if self.history["disk"] else 0,
                "gpu_avg": sum(self.history["gpu"]) / len(self.history["gpu"]) 
                if self.history["gpu"] else 0,
            }
        }
...
if __name__ == "__main__":
    ...
    if report["alerts"]:
        ...
        for alert in report["alerts"]:
            print(f"- {alert['resource']}: {alert['value']} (threshold: {alert['threshold']})")
    else:
