...
class BackupWorker:
    ...
    def __init__(
        ...
        self.backup_scripts = (
            backup_scripts
            if backup_scripts is not None
            else ["./scripts/backup_manager.sh"]
        )
        self.verify_scripts = (
            verify_scripts
            if verify_scripts is not None
            else ["./scripts/backup_verify.sh"]
        )
    ...
    def start(self) -> None:
        ...
        self.logger.info(
            "Starting backup worker with interval %d seconds", self.interval
        )
