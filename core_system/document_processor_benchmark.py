...
class DocumentProcessorBenchmark:
    ...
    def __init__(
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(
                    os.path.join(log_dir, "document_processor_benchmark.log")
                ),
    ...
    def benchmark_document_processing(
        self, file_paths: List[str], iterations: int = 3
    ) -> Dict[str, Any]:
        """
        ...
        for iteration in range(iterations):
            ...
            benchmark_results["system_metrics"]["memory_usage"].append(
                (start_memory + end_memory) / 2
            )
            benchmark_results["system_metrics"]["gpu_memory_usage"].append(
                gpu_memory_usage
            )
    ...
    def generate_performance_report(self, benchmark_results: Dict[str, Any]):
        ...
        print(
            f"Total Processing Time: {benchmark_results['statistical_summary']['total_processing_time']['mean']:.4f} ± {benchmark_results['statistical_summary']['total_processing_time']['stdev']:.4f} seconds"
        )
        print(
            f"Average CPU Usage: {benchmark_results['statistical_summary']['cpu_usage']['mean']:.2f}%"
        )
        print(
            f"Average Memory Usage: {benchmark_results['statistical_summary']['memory_usage']['mean']:.2f}%"
        )
