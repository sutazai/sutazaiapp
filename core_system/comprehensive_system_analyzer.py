...
class AdvancedSystemManager:
    ...
    def run_comprehensive_analysis(self) -> Dict[str, Any]:
        ...
        comprehensive_report['optimization_recommendations'] = self.generate_optimization_recommendations(comprehensive_report)
    ...
    def _detect_system_changes(self) -> Dict[str, Any]:
        ...
        for path, old_structure in self.system_state['initial_snapshot']['directories'].items():
            current_structure = current_snapshot['directories'].get(path, {})
            
            # Detect directory changes
            self._compare_directory_structures(old_structure, current_structure, changes)
    ...
    def _compare_directory_structures(
        ...
        old_subdirs = {k: v for k, v in old_structure.items() if k not in ['__files__', '__subdirs__']}
        current_subdirs = {k: v for k, v in current_structure.items() if k not in ['__files__', '__subdirs__']}
    ...
    def _log_and_persist_report(self, report: Dict[str, Any]):
        ...
        report_filename = f'system_analysis_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        ...
        logger.info(f"Security Issues Detected: {len(report['security_analysis'].get('potential_security_issues', []))}")
        logger.info(f"Optimization Recommendations: {len(report['optimization_recommendations'])}")
        ...
        try:
            for root, dirs, files in os.walk(self.base_dir):
                ...
                for file in files:
                    ...
                    # Track file types
                    system_components['file_types'][file_ext] = system_components['file_types'].get(file_ext, 0) + 1
        ...
        try:
            ...
            for file_path in python_files:
                try:
                    ...
                    # Store Pylint score
                    code_quality['pylint_scores'][file_path] = pylint_output.linter.stats.global_note
                except Exception as e:
        ...
        try:
            ...
            for file_path in python_files:
                try:
                    ...
                    logger.warning(f"Module graph generation failed for {file_path}: {e}")
    ...
    def generate_optimization_recommendations(
        ...
        low_quality_files = [
            file for file, score in code_quality.get('pylint_scores', {}).items()
        ...
        if dependency_analysis.get('python_dependencies'):
            outdated_deps = [
                dep for dep, info in dependency_analysis['python_dependencies'].items()
    ...
    def apply_recommendations(self, recommendations: List[str]):
        ...
        for recommendation in recommendations:
            try:
                ...
                logger.warning(f"Failed to apply recommendation: {recommendation}. Error: {e}")
...
"""
        ...
        markdown_path = os.path.join(self.output_dir, f'system_health_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.md')
