...
class AdvancedDocumentProcessor:
    ...
    def chunk_content(
        self, content: str, chunk_size: int = 512, overlap: int = 128
    ) -> List[str]:
        """
    ...
    async def process_document(
        self, file_path: str, chunk_size: int = 512, overlap: int = 128
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            content = await asyncio.to_thread(
                self._secure_content_extraction, file_path
            )

            # Chunking
            chunks = await asyncio.to_thread(
                self.chunk_content, content, chunk_size, overlap
            )
    ...
    def batch_process_documents(
        self, file_paths: List[str], chunk_size: int = 512, overlap: int = 128
    ) -> List[Dict[str, Any]]:
        """
