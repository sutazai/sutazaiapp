...
class PerformanceAnalyzer:
    ...
    def _generate_recommendations(self):
        ...
        for func in self.analysis_results['top_time_consuming_functions']:
            if func['cumulative_time'] > 0.5:  # Threshold for significant time consumption
                recommendations.append({
                    'function': func['function'],
                    'time_consumed': func['cumulative_time'],
                    'recommendation': self._get_optimization_suggestion(func['function'])
                })
    ...
    def print_analysis(self):
        ...
        print(f"Total Execution Time: {self.analysis_results['total_time']:.4f} seconds")
        
        print("\n📊 Top Time-Consuming Functions:")
        for i, func in enumerate(self.analysis_results['top_time_consuming_functions'], 1):
