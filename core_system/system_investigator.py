...
class SystemInvestigator:
                        ...
                        try:
                            with open(file_path, "r", encoding=encoding) as f:
                                try:
                                    ...
                                    logging.warning(
                                        f"Syntax error in {file_path} with {encoding} encoding: {e}"
                                    )
    ...
    def generate_system_report(self, duplicates: Dict[str, List[str]]):
        ...
        self.investigation_report.update(
            {
                "total_duplicate_files": sum(
                    len(files) for files in duplicates.values()
                ),
        ...
        print(
            f"Total Duplicate Files: {self.investigation_report['total_duplicate_files']}"
        )
        ...
        if self.investigation_report["code_quality_issues"]:
            ...
            for issue in self.investigation_report["code_quality_issues"]:
                print(
                    f"  - {issue['file']}: {issue['function']} (Complexity: {issue['complexity']})"
                )
