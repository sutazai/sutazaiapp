...
class SutazAiRenamer:
    ...
    def global_rename(self, root_dir='.'):
        ...
        try:
            ...
            for root, dirs, files in os.walk(root_dir, topdown=False):
                for dir_name in dirs:
                    if self.old_name.lower() in dir_name.lower():
                        ...
                        new_path = os.path.join(root, dir_name.replace(self.old_name.lower(), self.new_name.lower()))
            ...
            for root, _, files in os.walk(root_dir):
                for file in files:
                    ...
                    if self.old_name.lower() in file.lower():
                        new_file_name = file.replace(self.old_name.lower(), self.new_name.lower())
            ...
            self.logger.info(f"Renaming complete. Renamed {renamed_dirs} directories and {renamed_files} files.")
    ...
    def rename_entities(self, entities: List[str]) -> List[str]:
        ...
        """
        renamed_entities = [entity.replace(self.old_name, self.new_name) for entity in entities]
