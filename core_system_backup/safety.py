...
def get_from_cache(
    db_name: str,
    cache_valid_seconds: int = 0,
    skip_time_verification: bool = False,
...
def fetch_database_url(
    ...
    """
    headers = {
        "schema-version": JSON_SCHEMA_VERSION,
...
def get_cve_from(
    ...
    try:
        xve_id: str = str(
            next(
                filter(
                    lambda i: i.get("type", None) in ["cve", "pve"],
                    data.get("ids", []),
                )
            ).get("id", "")
        )
...
def get_closest_ver(
    ...
    """
    results: Dict[str, Optional[Union[str, Version]]] = {
        "upper": None,
        "lower": None,
    }
    ...
    for pkg_name in remediations.keys():
        ...
        for analyzed_requirement in analyzed:
            ...
            if not secure_vulns_by_user:
                secure_v = sorted(
                    secure_versions,
                    key=lambda ver: parse_version(ver),
                    reverse=True,
                )
            else:
...
def process_fixes_scan(
    ...
    def get_remmediation_from(spec):
        ...
        try:
            ...
            LOG.error(
                f"Error getting upper remediation version, ignoring",
                exc_info=True,
            )
        ...
        try:
            ...
            LOG.error(
                f"Error getting lower remediation version, ignoring",
                exc_info=True,
            )
...
def get_announcements(
    ...
    try:
        ...
        LOG.info(
            "Unexpected but HANDLED Exception happened getting the announcements: %s",
            e,
        )
        return announcements
