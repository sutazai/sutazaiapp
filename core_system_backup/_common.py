...
def memoize_when_activated(fun):
    ...
    setattr(wrapper, 'cache_activate', cache_activate)
    setattr(wrapper, 'cache_deactivate', cache_deactivate)
    return wrapper
...
def open_text(fname):
    ...
    # Buffer size is handled by the open() call above
    return fobj
...
def hilite(s: str, color: Optional[str] = None, bold: bool = False) -> str:
    ...
    # Handle color selection with proper type handling
    color_str = str(color) if color is not None else 'default'
    color_code = colors.get(color_str, '29')
...
def print_color(s: str, color: Optional[str] = None, bold: bool = False, file=sys.stdout) -> None:
    ...
    if not term_supports_colors():
        ...
        colors = {'green': 2, 'red': 4, 'brown': 6, 'yellow': 6}
        # Handle color selection with proper type handling
        color_str = str(color) if color is not None else 'default'
        color_code = colors.get(color_str, DEFAULT_COLOR)

        if bold and color_code <= 7:
            color_code += 8
...
def debug(msg):
    ...
    if PSUTIL_DEBUG:
        ...
        if frame is not None:
            frame = frame.f_back
            if frame is not None:
                fname, lineno, _, _lines, _index = inspect.getframeinfo(frame)
