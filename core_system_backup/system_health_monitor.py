#!/usr/bin/env python3
# cSpell:ignore Sutaz SutazAI sutazai levelname percpu
...
class UltraComprehensiveSystemHealthMonitor:
    ...
    def _handle_performance_alerts(self, alerts: List[Dict[str, Any]]):
        ...
        for alert in alerts:
            self.logger.warning(
                f"Performance Alert: {alert['type']} - Current: {alert['current_value']}%"
            )
...
def gather_system_health(
    health_data: Dict[str, float] = None,
) -> Dict[str, float]:
    """
    Gathers system health metrics.

    Args:
        health_data: Optional initial health data dictionary.

    Returns:
        A dictionary of health metrics with values as floats.
    """
    ...
    health_data["memory_usage"] = mem_usage
    health_data["disk_usage"] = disk_usage

    return health_data


def system_health_report() -> Dict[str, float]:
    """
    Builds a complete system health report.

    Returns:
        A dictionary of system health metrics (all floats).
    """
    report = gather_system_health()
...
def get_system_metrics(
    metrics: Optional[Dict[str, Union[float, list[float], int]]] = None,
) -> Dict[str, float]:
    if metrics is None:
        metrics = {}
...
class SystemHealthMonitor:
    ...
    def is_system_healthy(
        ...
        """Evaluate system health based on resource utilization"""
        metrics = SystemHealthMonitor.get_system_metrics()

        return all(
            [
                metrics.get("cpu_usage", 0) < cpu_threshold,
                metrics.get("memory_usage", 0) < memory_threshold,
                metrics.get("disk_usage", 0) < disk_threshold,
            ]
        )
