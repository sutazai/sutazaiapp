...
class TaskResponse(BaseModel):
    ...
    result: Any
    agent: str = "crewai"
    capabilities: List[str] = ['team_coordination', 'role_assignment', 'collaborative_tasks']

class AgentInfo(BaseModel):
    id: str = "crewai"
    name: str = "CrewAI"
    description: str = "Multi-agent collaboration framework"
    capabilities: List[str] = ['team_coordination', 'role_assignment', 'collaborative_tasks']
...
@app.get("/")
async def root():
    return {"agent": "CrewAI", "status": "active"}
...
async def process_task(request: TaskRequest) -> Dict[str, Any]:
    ...
    if "crewai" == "autogpt" and "autonomous" in task:
        return await handle_autonomous_task(request)
    elif "crewai" == "crewai" and "team" in task:
        return await handle_team_task(request)
    elif "crewai" == "aider" and "code" in task:
...
async def handle_generic_task(request: TaskRequest) -> Dict[str, Any]:
    ...
    return {
        "task": request.task,
        "agent": "crewai",
        "result": f"Task processed by CrewAI"
    }
