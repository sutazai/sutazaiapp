...
including task processing, system_state, and learning capabilities.
"""
...
coordinator_URL = "http://localhost:8001"  # Assuming coordinator runs on separate port
coordinator_INTERNAL_URL = "http://sutazai-coordinator:8001"  # Docker internal URL
...
async def coordinator_client():
    ...
    for url in [coordinator_URL, coordinator_INTERNAL_URL]:
        try:
...
class TestprocessingProcessingIntegration:
    """Test task processing integration between backend and coordinator"""
    
    @pytest.mark.asyncio
    async def test_processing_processing_endpoint(self, backend_client, auth_headers):
        """Test task processing through backend"""
        payload = {
            "input_data": {
        ...
        # If task processing is actually working
        if not data.get("fallback_mode", False):
            assert "processing_pathways_activated" in data
    
    @pytest.mark.asyncio
    async def test_system_state_monitoring(self, backend_client, auth_headers):
    ...
    async def test_creative_synthesis_integration(self, backend_client, auth_headers):
        """Test creative synthesis task processing"""
        payload = {
...
class Testcoordinatorcontinuous_improvementIntegration:
    """Test continuous_improvement system integration"""
    ...
    async def test_legacy_improvement_system(self, backend_client, auth_headers):
        """Test legacy continuous_improvement endpoint"""
...
class TestcoordinatorPerformanceIntegration:
    ...
    async def test_processing_processing_performance(self, backend_client, auth_headers):
        ...
        assert avg_time < 30.0, f"Average task processing time too high: {avg_time}s"
        assert max_time < 45.0, f"Maximum task processing time too high: {max_time}s"
        
        print(f"task processing performance - Avg: {avg_time:.2f}s, Max: {max_time:.2f}s, Min: {min_time:.2f}s")
...
class TestcoordinatorErrorHandling:
    ...
    async def test_invalid_processing_processing_input(self, backend_client, auth_headers):
        """Test handling of invalid task processing input"""
        invalid_payloads = [
            {},  # Empty payload
