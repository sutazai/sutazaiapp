...
class TestprocessingProcessingEndpoints:
    """Test task processing and system_state endpoints"""
...
class Testcontinuous_improvementEndpoints:
    """Test continuous_improvement system endpoints"""
    ...
    async def test_improvement_analyze(self, client, auth_headers):
        ...
        data = response.json()
        # Response structure depends on whether continuous_improvement system is available
        assert "timestamp" in data
        
        # If enterprise continuous_improvement is available
