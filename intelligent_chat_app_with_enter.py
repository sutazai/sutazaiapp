...
Real-time intelligent chatbot with Enter key support and voice features
"""
...
function setupEnterKeyListener() {
    ...
    function handleKeyDown(event) {
        // Check if Enter key is pressed (not Shift+Enter)
        if (event.key === 'Enter' && !event.shiftKey) {
            // Find the active textarea (chat input)
            const activeElement = document.activeElement;
            if (activeElement && activeElement.tagName === 'TEXTAREA') {
                // Find the send button (look for primary button)
...
// Also set up listener after any Streamlit updates
window.addEventListener('load', setupEnterKeyListener);

// Retry setup periodically to handle Streamlit's dynamic updates
...
if 'enter_key_enabled' not in st.session_state:
    st.session_state.enter_key_enabled = True
...
def listen_for_speech():
    ...
    try:
        ...
        st.error("❌ Voice recognition not available. Please install required dependencies.")
        return None
    except Exception as e:
        st.warning(f"⚠️ Voice recognition error: {e}")
        return None
...
def speak_text(text: str):
    ...
    try:
        ...
        st.error("❌ Text-to-speech not available. Please install required dependencies.")
    except Exception as e:
...
def display_header():
    ...
    <div style="text-align: center; padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 15px; margin-bottom: 30px;">
        ...
        <p style="color: white; margin: 10px 0 0 0; font-size: 1.2em;">Advanced AI System with Enter Key & Voice Features</p>
    </div>
    """, unsafe_allow_html=True)

def display_voice_controls():
    """Display voice control interface"""
    st.markdown("### 🎤 Voice & Input Controls")
    ...
    with col3:
        st.session_state.enter_key_enabled = st.checkbox(
            "⌨️ Enter Key", 
            value=st.session_state.enter_key_enabled,
            help="Enable Enter key to send messages"
        )
...
def display_chat_interface():
    ...
    with st.container():
        ...
        # Enter key hint - only show if enabled
        if st.session_state.enter_key_enabled:
            st.markdown("""
            <div class="enter-hint">
                💡 <strong>Enter Key Enabled!</strong> Press Enter to send your message, Shift+Enter for new line
            </div>
            """, unsafe_allow_html=True)
        ...
        with col4:
            if st.button("🤖 Test Voice", use_container_width=True):
                if st.session_state.voice_enabled:
                    speak_text("Hello! Voice output is working correctly.")
                else:
                    st.session_state.chat_history.append({
                        "role": "system",
                        "content": "Voice output is disabled. Enable it in the controls above.",
                        "timestamp": datetime.now().isoformat()
                    })
                    st.rerun()
    ...
    if st.session_state.chat_history:
        ...
        st.markdown("---")
        st.info("👋 Welcome! Start a conversation by typing a message above and pressing Enter, or use voice input. I'm your intelligent AI assistant ready to help with any task!")
...
def display_sidebar():
    ...
    enter_status = "✅ Enabled" if st.session_state.enter_key_enabled else "❌ Disabled"
    voice_status = "🔊 Enabled" if st.session_state.voice_enabled else "🔇 Disabled"
    st.sidebar.markdown(f"**Enter Key:** {enter_status}")
    ...
    if st.session_state.backend_url:
        ...
        if "error" not in status:
            ...
            st.sidebar.text("⌨️ Enter Key: Available")
        else:
    ...
    sample_commands = [
        ...
        "Tell me about Enter key feature",
    ...
    st.sidebar.markdown("*Intelligent AGI/ASI Platform*")
    ...
    st.sidebar.markdown(f"**Features:** {'⌨️' if st.session_state.enter_key_enabled else ''}{'🎤' if st.session_state.voice_enabled else ''}")
