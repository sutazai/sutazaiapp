...
import json
import threading
from typing import Dict, List, Any, Optional, SupportsFloat
...
class HardwareMonitor:
    ...
    def collect_metrics(self) -> Dict[str, Any]:
        ...
        if PSUTIL_AVAILABLE and self.hardware_detected["cpu"]["available"]:
            try:
                metrics["cpu"]["usage_percent"] = psutil.cpu_percent(interval=None)
                cpu_times = psutil.cpu_times_percent()
                metrics["cpu"]["times_percent"] = {
                    "user": cpu_times.user,
                ...
                self.logger.debug(f"Could not get CPU metrics: {e}")
        ...
        if PSUTIL_AVAILABLE and self.hardware_detected["memory"]["available"]:
            try:
                ...
                self.logger.debug(f"Could not get memory metrics: {e}")
        ...
        if PSUTIL_AVAILABLE and self.hardware_detected["disk"]["available"]:
            ...
            try:
                ...
                for disk_name, counters in disk_io.items():
                    ...
                    # Rates require tracking previous values
            except Exception as e:
        ...
        if PSUTIL_AVAILABLE and self.hardware_detected["network"]["available"]():
            try:
                ...
                for interface, snetio in net_io.items():
        ...
        # Assign collected metrics to the main metrics dict
