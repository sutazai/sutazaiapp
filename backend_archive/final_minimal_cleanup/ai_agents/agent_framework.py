        ...
        try:
            ...
            if config.tools:
                for tool_name in config.tools:
                    ...
                    # Remove redundant check for tool_config.function truthiness
                    if tool_config and hasattr(tool_config, 'function'):
                        # Check if the function attribute is actually callable
                        if callable(tool_config.function):
                             agent_tools.append(
                                 Tool(
                             ...
                             logger.warning(f"Tool implementation for '{tool_name}' is not callable.")
                    else:
                        missing_tool_methods.append(tool_name)
                        logger.warning(f"Tool implementation or definition for '{tool_name}' not found.")
                
                # This check should be outside the loop
                ...
                # Log mapped tools outside the loop
                if agent_tools:
        ...
        try:
            ...
            class CustomAgentImpl(BaseAgent):
                ...
                def _execute(self, task: Dict[str, Any]) -> Dict[str, Any]:
                    """Execute a task using the custom agent's logic (synchronous)."""
                    ...
                    try:
                        ...
                        # Assume ModelManager has a synchronous run_inference method
                        if hasattr(self.model_manager, 'run_inference'):
                             ...
                             # If no sync method, raise error or handle differently
                             raise NotImplementedError("ModelManager does not have a synchronous run_inference method.")
