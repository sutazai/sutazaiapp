...
def extract_archive(path: Path, dest_root: Path) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    ...
    try:
        if kind == 'zip':
            ...
            note = f'Unsupported archive type: {kind}'
    except Exception as e:
        note = f'Extraction error: {e}'
...
def generate_inventory():
    ...
    for f in files:
        ...
        if is_archive(f):
            extracted, note = extract_archive(f, EXTRACT_DIR)
    ...
    with open(matrix_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        writer.writerow(['path', 'from_line', 'to_line', 'status', 'notes', 'source_of_truth_ref'])
        for f in files:
            if f.suffix.lower() not in DOC_EXTS:
                continue
            rel = str(f.relative_to(ROOT))
            content_lines, note = load_text_for_review(f)
            if content_lines:
                block = 100
                ...
                for i in range(0, total, block):
                    ...
                    status = '⚠️unclear'
                    writer.writerow([rel, start, end, status, note or '', ''])
            else:
                # Single block placeholder when extraction missing
                writer.writerow([rel, 1, 1, '🕳️missing', note or 'Binary/unparsed file', ''])
