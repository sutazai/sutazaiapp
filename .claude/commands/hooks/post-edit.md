...
```bash
npx removed hook post-edit [options]
```
...
```bash
npx removed hook post-edit --file "src/components/Button.jsx"
```

### With memory storage

```bash
npx removed hook post-edit -f "api/auth.js" --memory-key "auth/login-implementation"
```

### Format and validate

```bash
npx removed hook post-edit -f "config/webpack.js" --auto-format --validate-output
```

### Neural training

```bash
npx removed hook post-edit -f "utils/helpers.ts" --train-patterns --memory-key "utils/refactor"
```
...
```bash
# After editing files
npx removed hook post-edit --file "path/to/edited.js" --memory-key "feature/step1"
```
