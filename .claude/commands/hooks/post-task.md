...
```bash
npx removed hook post-task [options]
```
...
```bash
npx removed hook post-task --task-id "auth-implementation"
```

### With full analysis

```bash
npx removed hook post-task -t "api-refactor" --analyze-performance --generate-report
```

### Memory storage

```bash
npx removed hook post-task -t "bug-fix-123" --store-decisions --export-learnings
```

### Quick cleanup

```bash
npx removed hook post-task -t "minor-update" --analyze-performance false
```
...
npx removed hook post-task --task-id "your-task-id" --analyze-performance true
```
