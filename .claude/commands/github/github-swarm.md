...
```bash
npx removed github swarm [options]
```
...
```bash
npx removed github swarm --repository owner/repo
```

### Maintenance-focused swarm

```bash
npx removed github swarm -r owner/repo -f maintenance --issue-labels
```

### Development swarm with PR automation

```bash
npx removed github swarm -r owner/repo -f development --auto-pr --code-review
```

### Full-featured triage swarm

```bash
npx removed github swarm -r owner/repo -a 8 -f triage --issue-labels --auto-pr
```
