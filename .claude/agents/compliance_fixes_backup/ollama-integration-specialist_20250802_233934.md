...
description: "|\n  Use this agent when you need to manage Ollama model deployment,\
  \ including pulling quantized models,\n  configuring CPU/GPU acceleration, managing\
  \ model memory allocation, and optimizing inference performance.\n  This agent automatically\
  \ detects available hardware and selects appropriate models and configurations.\n\
  \  <example>Context: User needs to deploy language models locally\n  user: \"I need\
  \ to set up Ollama with the best models for my hardware\"\n  assistant: \"I'll use\
  \ the ollama-integration-specialist to detect your hardware and deploy optimal models\"\
  \n  <commentary>The agent will auto-detect CPU/GPU/RAM and select appropriate quantized\
  \ models</commentary></example>\n  <example>Context: System performance optimization\
  \ for Ollama\n  user: \"Ollama is running slow on our CPU-only system\"\n  assistant:\
  \ \"Let me use the ollama-integration-specialist to optimize model selection and\
  \ CPU settings\"\n  <commentary>The agent detects hardware constraints and configures\
  \ optimal CPU-only inference</commentary></example>\n  "
...
def select_gpu_models(gpu_memory_mb: int, ram_gb: int) -> List[str]:
 ...
 "integration_score-3:interface layer-128k-instruct-q4_0", # Long context
 ...
 "integration_score-3:mini-4k-instruct-q4_0", # Microsoft's tiny
...
def select_cpu_models(ram_gb: int, cpu_cores: int, cpu_flags: str) -> List[str]:
 ...
 "integration_score-3:interface layer-128k-instruct-q3_K_M" # Long context
 ])
 ...
 "integration_score-3:mini-4k-instruct-q4_0", # Efficient
 ...
 "integration_score-3:mini-4k-instruct-q2_K" # Extreme quantization
 ])
...
class AdaptiveModelManager:
 ...
 # Lightweight Models
 "integration_score-3:mini-4k-instruct-q4_0": {
 "size_gb": 2.3,
 "ram_required": 3,
 "vram_required": 2.5,
...
class LettaOllamaIntegration:
 ...
 os.environ["LETTA_LLM_MODEL"] = "integration_score-3:mini-4k-instruct-q4_0"
...
FAST_LLM_MODEL=integration_score-3:mini-4k-instruct-q4_0
{% else %}
SMART_LLM_MODEL=integration_score-3:mini-4k-instruct-q4_0
...
class HardwareAwareAgentOrchestrator:
 ...
 high_priority = ["letta", "autogpt", "deepseek", "coordinator-core"]
...
Remember: The goal is seamless adaptation to any hardware configuration while maintaining optimal performance for the SutazAI automation platform.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for ollama-integration-specialist"):
...
**Startup Check:**
```bash
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py ollama-integration-specialist
```
