...
name: resource-visualiser-detailed
...
## Purpose
Specialized AI agent for resource visualiser
...
#!/usr/bin/env python3
"""
Resource Visualiser - Comprehensive automation platform Agent Implementation
Agent for resource visualiser
"""
...
logger = logging.getLogger('resource_visualiser')
...
class ResourceVisualiser(ComprehensiveSystemInvestigator):
 """
 Resource Visualiser Implementation
 
 This agent specialized ai agent for resource visualiser
 """
 ...
 logger.info(f"Initialized resource-visualiser with profile: {self.hardware_profile}")
...
def main():
 ...
 parser = argparse.ArgumentParser(description='Resource Visualiser')
 ...
 # Initialize agent
 agent = ResourceVisualiser()
 
 if args.command == 'start':
 logger.info(f"Starting resource-visualiser...")
 # Start agent service
...
```yaml
# docker-compose-resource-visualiser.yml
version: '3.8'
...
services:
 ...
 build:
 context: .
 dockerfile: Dockerfile.resource-visualiser
 ...
 memory: 4G
 healthcheck:
 test: ["CMD", "python", "-m", "agents.resource_visualiser", "status"]
 interval: 30s
 timeout: 10s
 retries: 3
```
...
```bash
# Direct execution
python -m agents.resource_visualiser start

# Docker deployment
docker-compose -f docker-compose-resource-visualiser.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.resource_visualiser status
...
```bash
python -m agents.resource_visualiser test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.resource_visualiser investigate
```
...
This comprehensive implementation ensures the resource-visualiser agent operates efficiently within the SutazAI system while maintaining the conservative resource strategy.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for resource-visualiser-detailed"):
...
- `AGENT_NAME=resource-visualiser-detailed`

**Startup Check:**
```bash
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py resource-visualiser-detailed
```
