...
name: flowiseai-flow-manager-detailed
...
## Purpose
Specialized AI agent for flowiseai flow manager
...
#!/usr/bin/env python3
"""
Flowiseai Flow Manager - Comprehensive automation platform Agent Implementation
...
logger = logging.getLogger('flowiseai_flow_manager')
...
class FlowiseaiFlowManager(ComprehensiveSystemInvestigator):
 """
 Flowiseai Flow Manager Implementation
 
 This agent specialized ai agent for flowiseai flow manager
 """
 ...
 logger.info(f"Initialized flowiseai-flow-manager with profile: {self.hardware_profile}")
...
def main():
 ...
 parser = argparse.ArgumentParser(description='Flowiseai Flow Manager')
 ...
 # Initialize agent
 agent = FlowiseaiFlowManager()
 
 if args.command == 'start':
 logger.info(f"Starting flowiseai-flow-manager...")
 # Start agent service
...
```yaml
# docker-compose-flowiseai-flow-manager.yml
version: '3.8'
...
services:
 ...
 build:
 context: .
 dockerfile: Dockerfile.flowiseai-flow-manager
 ...
 memory: 4G
 healthcheck:
 test: ["CMD", "python", "-m", "agents.flowiseai_flow_manager", "status"]
 interval: 30s
 timeout: 10s
 retries: 3
```
...
```bash
# Direct execution
python -m agents.flowiseai_flow_manager start

# Docker deployment
docker-compose -f docker-compose-flowiseai-flow-manager.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.flowiseai_flow_manager status
...
```bash
python -m agents.flowiseai_flow_manager test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.flowiseai_flow_manager investigate
```
...
This comprehensive implementation ensures the flowiseai-flow-manager agent operates efficiently within the SutazAI system while maintaining the conservative resource strategy.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for flowiseai-flow-manager-detailed"):
...
- `AGENT_NAME=flowiseai-flow-manager-detailed`

**Startup Check:**
```bash
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py flowiseai-flow-manager-detailed
```
