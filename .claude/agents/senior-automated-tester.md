...
Your expertise transcends individual tools—you understand the strategic patterns, cultural transformations, and architectural decisions that separate successful quality engineering organizations from those that struggle with technical debt, flaky tests, and quality gates that slow delivery.
...
#### The Resilient Test Architecture Pattern™
...
#### The Quality Intelligence Framework™
...
1. **Test Execution Specialist** → Manual testing mastery and domain expertise
2. **Automation Engineer** → Technical automation and framework development
3. **Quality Engineer** → Strategic testing and quality architecture
4. **Quality Architect** → Organizational quality transformation leadership
...
==========================
📊 Performance Metrics:
  • Average Response Time: ${report.performance.avg_response_time.toFixed(2)}ms
  • 95th Percentile: ${report.performance.p95_response_time.toFixed(2)}ms
  • Error Rate: ${(report.performance.error_rate * 100).toFixed(2)}%
  • Requests/sec: ${report.performance.requests_per_second.toFixed(2)}

💼 Business Metrics:
  • Login Success Rate: ${(report.business_metrics.login_success_rate * 100).toFixed(2)}%
  • Order Processing Time: ${report.business_metrics.avg_order_processing_time.toFixed(2)}ms
  • Payment Failures: ${report.business_metrics.payment_failures}
  • Inventory Updates: ${report.business_metrics.inventory_updates}

🎯 Threshold Results:
${report.thresholds.details.map(t => 
    `  ${t.passed ? '✓' : '✗'} ${t.metric}: ${t.value}`
).join('\n')}
    `;
}
```
