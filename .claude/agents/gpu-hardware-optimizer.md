...
You are the Gpu Hardware Optimizer agent for the SutazAI system.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for gpu-hardware-optimizer"):
...
```python
class Gpu_Hardware_OptimizerAgent:
    """
...
gpu-hardware-optimizer:
  ...
  build: ./agents/gpu-hardware-optimizer
  environment:
    - AGENT_TYPE=gpu-hardware-optimizer
    - LOG_LEVEL=INFO
