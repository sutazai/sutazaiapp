...
name: ram-hardware-optimizer-detailed
...
## Purpose
Specialized AI agent for ram hardware optimizer
...
#!/usr/bin/env python3
"""
Ram Hardware Optimizer - Comprehensive automation platform Agent Implementation
Agent for ram hardware optimizer
"""
...
logger = logging.getLogger('ram_hardware_optimizer')
...
class RamHardwareOptimizer(ComprehensiveSystemInvestigator):
 """
 Ram Hardware Optimizer Implementation
 
 This agent specialized ai agent for ram hardware optimizer
 """
 ...
 logger.info(f"Initialized ram-hardware-optimizer with profile: {self.hardware_profile}")
...
def main():
 ...
 parser = argparse.ArgumentParser(description='Ram Hardware Optimizer')
 ...
 # Initialize agent
 agent = RamHardwareOptimizer()
 
 if args.command == 'start':
 logger.info(f"Starting ram-hardware-optimizer...")
 # Start agent service
...
```yaml
# docker-compose-ram-hardware-optimizer.yml
version: '3.8'
...
services:
 ...
 memory: 4G
 healthcheck:
 test: ["CMD", "python", "-m", "agents.ram_hardware_optimizer", "status"]
 interval: 30s
 timeout: 10s
 retries: 3
```
...
```bash
# Direct execution
python -m agents.ram_hardware_optimizer start

# Docker deployment
docker-compose -f docker-compose-ram-hardware-optimizer.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.ram_hardware_optimizer status
...
```bash
python -m agents.ram_hardware_optimizer test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.ram_hardware_optimizer investigate
```
...
This comprehensive implementation ensures the ram-hardware-optimizer agent operates efficiently within the SutazAI system while maintaining the conservative resource strategy.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for ram-hardware-optimizer-detailed"):
...
- `AGENT_NAME=ram-hardware-optimizer-detailed`

**Startup Check:**
```bash
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py ram-hardware-optimizer-detailed
```
