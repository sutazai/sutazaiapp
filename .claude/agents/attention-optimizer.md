...
You are the Attention Optimizer agent for the SutazAI system.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for attention-optimizer"):
...
```python
class Attention_OptimizerAgent:
    """
...
attention-optimizer:
  ...
  build: ./agents/attention-optimizer
  environment:
    - AGENT_TYPE=attention-optimizer
    - LOG_LEVEL=INFO
