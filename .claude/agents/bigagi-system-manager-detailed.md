...
## Purpose
Specialized AI agent for bigagi system manager
...
#!/usr/bin/env python3
"""
Bigagi System Manager - Comprehensive AGI Agent Implementation
...
logger = logging.getLogger('bigagi_system_manager')
...
class BigagiSystemManager(ComprehensiveSystemInvestigator):
    """
    Bigagi System Manager Implementation
    
    This agent specialized ai agent for bigagi system manager
    """
    ...
    def __init__(self, brain_path: str = "/opt/sutazaiapp/brain"):
        ...
        logger.info(f"Initialized bigagi-system-manager with profile: {self.hardware_profile}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Bigagi System Manager')
    ...
    # Initialize agent
    agent = BigagiSystemManager()
    
    if args.command == 'start':
        logger.info(f"Starting bigagi-system-manager...")
        # Start agent service
...
```yaml
# docker-compose-bigagi-system-manager.yml
version: '3.8'
...
services:
  bigagi-system-manager:
    build:
      context: .
      dockerfile: Dockerfile.bigagi-system-manager
    ...
    deploy:
      resources:
        limits:
          ...
          memory: 4G
    healthcheck:
      test: ["CMD", "python", "-m", "agents.bigagi_system_manager", "status"]
      interval: 30s
      timeout: 10s
      retries: 3
```
...
```bash
# Direct execution
python -m agents.bigagi_system_manager start

# Docker deployment
docker-compose -f docker-compose-bigagi-system-manager.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.bigagi_system_manager status
...
```bash
python -m agents.bigagi_system_manager test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.bigagi_system_manager investigate
```
...
This comprehensive implementation ensures the bigagi-system-manager agent operates efficiently within the SutazAI advanced AI system while maintaining the conservative resource strategy.