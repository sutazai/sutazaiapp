...
name: knowledge-graph-builder-detailed
...
## Purpose
Specialized AI agent for knowledge graph builder
...
#!/usr/bin/env python3
"""
Knowledge Graph Builder - Comprehensive automation platform Agent Implementation
...
class KnowledgeGraphBuilder(ComprehensiveSystemInvestigator):
 """
 Knowledge Graph Builder Implementation
 
 This agent specialized ai agent for knowledge graph builder
 """
 ...
 logger.info(f"Initialized knowledge-graph-builder with profile: {self.hardware_profile}")
...
def main():
 ...
 logger.info(f"Starting knowledge-graph-builder...")
 # Start agent service
...
```yaml
# docker-compose-knowledge-graph-builder.yml
version: '3.8'
...
services:
 ...
 build:
 context: .
 dockerfile: Dockerfile.knowledge-graph-builder
 ...
 memory: 4G
 healthcheck:
 test: ["CMD", "python", "-m", "agents.knowledge_graph_builder", "status"]
 interval: 30s
 timeout: 10s
 retries: 3
```
...
```bash
# Direct execution
python -m agents.knowledge_graph_builder start

# Docker deployment
docker-compose -f docker-compose-knowledge-graph-builder.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.knowledge_graph_builder status
...
```bash
python -m agents.knowledge_graph_builder test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.knowledge_graph_builder investigate
```
...
This comprehensive implementation ensures the knowledge-graph-builder agent operates efficiently within the SutazAI system while maintaining the conservative resource strategy.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for knowledge-graph-builder-detailed"):
...
- `AGENT_NAME=knowledge-graph-builder-detailed`

**Startup Check:**
```bash
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py knowledge-graph-builder-detailed
```
