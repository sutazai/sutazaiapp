...
name: localagi-orchestration-manager-detailed
...
## Purpose
Specialized AI agent for localagi orchestration manager
...
#!/usr/bin/env python3
"""
Localagi Orchestration Manager - Comprehensive automation platform Agent Implementation
...
logger = logging.getLogger('localagi_orchestration_manager')
...
class LocalagiOrchestrationManager(ComprehensiveSystemInvestigator):
 """
 Localagi Orchestration Manager Implementation
 
 This agent specialized ai agent for localagi orchestration manager
 """
 ...
 logger.info(f"Initialized localagi-orchestration-manager with profile: {self.hardware_profile}")
...
def main():
 ...
 parser = argparse.ArgumentParser(description='Localagi Orchestration Manager')
 ...
 # Initialize agent
 agent = LocalagiOrchestrationManager()
 
 if args.command == 'start':
 logger.info(f"Starting localagi-orchestration-manager...")
 # Start agent service
...
```yaml
# docker-compose-localagi-orchestration-manager.yml
version: '3.8'
...
services:
 ...
 build:
 context: .
 dockerfile: Dockerfile.localagi-orchestration-manager
 ...
 memory: 4G
 healthcheck:
 test: ["CMD", "python", "-m", "agents.localagi_orchestration_manager", "status"]
 interval: 30s
 timeout: 10s
 retries: 3
```
...
```bash
# Direct execution
python -m agents.localagi_orchestration_manager start

# Docker deployment
docker-compose -f docker-compose-localagi-orchestration-manager.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.localagi_orchestration_manager status
...
```bash
python -m agents.localagi_orchestration_manager test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.localagi_orchestration_manager investigate
```
...
This comprehensive implementation ensures the localagi-orchestration-manager agent operates efficiently within the SutazAI system while maintaining the conservative resource strategy.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for localagi-orchestration-manager-detailed"):
...
- `AGENT_NAME=localagi-orchestration-manager-detailed`

**Startup Check:**
```bash
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py localagi-orchestration-manager-detailed
```
