...
name: context-optimization-engineer-detailed
...
## Purpose
Specialized AI agent for context optimization engineer
...
#!/usr/bin/env python3
"""
Context Optimization Engineer - Comprehensive automation platform Agent Implementation
...
logger = logging.getLogger('context_optimization_engineer')
...
class ContextOptimizationEngineer(ComprehensiveSystemInvestigator):
 """
 Context Optimization Engineer Implementation
 
 This agent specialized ai agent for context optimization engineer
 """
 ...
 logger.info(f"Initialized context-optimization-engineer with profile: {self.hardware_profile}")
...
def main():
 ...
 parser = argparse.ArgumentParser(description='Context Optimization Engineer')
 ...
 # Initialize agent
 agent = ContextOptimizationEngineer()
 
 if args.command == 'start':
 logger.info(f"Starting context-optimization-engineer...")
 # Start agent service
...
```yaml
# docker-compose-context-optimization-engineer.yml
version: '3.8'
...
services:
 ...
 build:
 context: .
 dockerfile: Dockerfile.context-optimization-engineer
 ...
 memory: 4G
 healthcheck:
 test: ["CMD", "python", "-m", "agents.context_optimization_engineer", "status"]
 interval: 30s
 timeout: 10s
 retries: 3
```
...
```bash
# Direct execution
python -m agents.context_optimization_engineer start

# Docker deployment
docker-compose -f docker-compose-context-optimization-engineer.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.context_optimization_engineer status
...
```bash
python -m agents.context_optimization_engineer test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.context_optimization_engineer investigate
```
...
This comprehensive implementation ensures the context-optimization-engineer agent operates efficiently within the SutazAI system while maintaining the conservative resource strategy.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for context-optimization-engineer-detailed"):
...
- `AGENT_NAME=context-optimization-engineer-detailed`

**Startup Check:**
```bash
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py context-optimization-engineer-detailed
```
