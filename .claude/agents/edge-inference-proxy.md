...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for edge-inference-proxy"):
...
```python
class Edge_Inference_ProxyAgent:
    """
...
edge-inference-proxy:
  ...
  build: ./agents/edge-inference-proxy
  environment:
    - AGENT_TYPE=edge-inference-proxy
    - LOG_LEVEL=INFO
