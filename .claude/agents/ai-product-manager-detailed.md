...
name: ai-product-manager-detailed
...
## Purpose
Specialized AI agent for ai product manager
...
#!/usr/bin/env python3
"""
Ai Product Manager - Comprehensive automation platform Agent Implementation
...
logger = logging.getLogger('ai_product_manager')
...
class AiProductManager(ComprehensiveSystemInvestigator):
 """
 Ai Product Manager Implementation
 
 This agent specialized ai agent for ai product manager
 """
 ...
 logger.info(f"Initialized ai-product-manager with profile: {self.hardware_profile}")
...
def main():
 ...
 parser = argparse.ArgumentParser(description='Ai Product Manager')
 ...
 logger.info(f"Starting ai-product-manager...")
 # Start agent service
...
```yaml
# docker-compose-ai-product-manager.yml
version: '3.8'
...
services:
 ...
 build:
 context: .
 dockerfile: Dockerfile.ai-product-manager
 ...
 memory: 4G
 healthcheck:
 test: ["CMD", "python", "-m", "agents.ai_product_manager", "status"]
 interval: 30s
 timeout: 10s
 retries: 3
```
...
```bash
# Direct execution
python -m agents.ai_product_manager start

# Docker deployment
docker-compose -f docker-compose-ai-product-manager.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.ai_product_manager status
...
```bash
python -m agents.ai_product_manager test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.ai_product_manager investigate
```
...
This comprehensive implementation ensures the ai-product-manager agent operates efficiently within the SutazAI system while maintaining the conservative resource strategy.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for ai-product-manager-detailed"):
...
- `AGENT_NAME=ai-product-manager-detailed`

**Startup Check:**
```bash
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py ai-product-manager-detailed
```
