...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for resource-visualiser"):
...
```python
class Resource_VisualiserAgent:
    """
...
resource-visualiser:
  ...
  build: ./agents/resource-visualiser
  environment:
    - AGENT_TYPE=resource-visualiser
    - LOG_LEVEL=INFO
