...
name: senior-ai-engineer-detailed
...
## Purpose
Specialized AI agent for senior ai engineer
...
#!/usr/bin/env python3
"""
Senior Ai Engineer - Comprehensive automation platform Agent Implementation
...
logger = logging.getLogger('senior_ai_engineer')
...
class SeniorAiEngineer(ComprehensiveSystemInvestigator):
 """
 Senior Ai Engineer Implementation
 
 This agent specialized ai agent for senior ai engineer
 """
 ...
 logger.info(f"Initialized senior-ai-engineer with profile: {self.hardware_profile}")
...
def main():
 ...
 parser = argparse.ArgumentParser(description='Senior Ai Engineer')
 ...
 # Initialize agent
 agent = SeniorAiEngineer()
 
 if args.command == 'start':
 logger.info(f"Starting senior-ai-engineer...")
 # Start agent service
...
```yaml
# docker-compose-senior-ai-engineer.yml
version: '3.8'
...
services:
 ...
 build:
 context: .
 dockerfile: Dockerfile.senior-ai-engineer
 ...
 memory: 4G
 healthcheck:
 test: ["CMD", "python", "-m", "agents.senior_ai_engineer", "status"]
 interval: 30s
 timeout: 10s
 retries: 3
```
...
```bash
# Direct execution
python -m agents.senior_ai_engineer start

# Docker deployment
docker-compose -f docker-compose-senior-ai-engineer.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.senior_ai_engineer status
...
```bash
python -m agents.senior_ai_engineer test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.senior_ai_engineer investigate
```
...
This comprehensive implementation ensures the senior-ai-engineer agent operates efficiently within the SutazAI system while maintaining the conservative resource strategy.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for senior-ai-engineer-detailed"):
...
- `AGENT_NAME=senior-ai-engineer-detailed`

**Startup Check:**
```bash
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py senior-ai-engineer-detailed
```
