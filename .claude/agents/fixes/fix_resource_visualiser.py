#!/usr/bin/env python3
"""
Compliance Fix for resource-visualiser
...
def fix_resource_visualiser():
    """Fix compliance issues for resource-visualiser"""
    
    agent_file = Path('/opt/sutazaiapp/.claude/agents/resource-visualiser.md')
...
def example_agent_action():
    action_desc = "Analyzing codebase for resource-visualiser tasks"
...
- `AGENT_NAME=resource-visualiser` - This agent's name for logging

### Startup Compliance Check:
```bash
# Use the startup wrapper to ensure compliance
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py resource-visualiser
```
'''
    ...
    if modified:
        with open(agent_file, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"✅ Fixed resource-visualiser")
        return True
    else:
        print(f"ℹ️  resource-visualiser already compliant")
        return True

if __name__ == "__main__":
    success = fix_resource_visualiser()
