#!/usr/bin/env python3
"""
Compliance Fix for task-assignment-coordinator-detailed
...
def fix_task_assignment_coordinator_detailed():
    """Fix compliance issues for task-assignment-coordinator-detailed"""
    
    agent_file = Path('/opt/sutazaiapp/.claude/agents/task-assignment-coordinator-detailed.md')
...
def example_agent_action():
    action_desc = "Analyzing codebase for task-assignment-coordinator-detailed tasks"
...
- `AGENT_NAME=task-assignment-coordinator-detailed` - This agent's name for logging
...
'''
    ...
    if modified:
        with open(agent_file, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"✅ Fixed task-assignment-coordinator-detailed")
        return True
    else:
        print(f"ℹ️  task-assignment-coordinator-detailed already compliant")
        return True

if __name__ == "__main__":
    success = fix_task_assignment_coordinator_detailed()
