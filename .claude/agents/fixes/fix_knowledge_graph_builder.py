#!/usr/bin/env python3
"""
Compliance Fix for knowledge-graph-builder
...
def fix_knowledge_graph_builder():
    """Fix compliance issues for knowledge-graph-builder"""
    
    agent_file = Path('/opt/sutazaiapp/.claude/agents/knowledge-graph-builder.md')
...
def example_agent_action():
    action_desc = "Analyzing codebase for knowledge-graph-builder tasks"
...
- `AGENT_NAME=knowledge-graph-builder` - This agent's name for logging
...
'''
    ...
    if modified:
        with open(agent_file, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"✅ Fixed knowledge-graph-builder")
        return True
    else:
        print(f"ℹ️  knowledge-graph-builder already compliant")
        return True

if __name__ == "__main__":
    success = fix_knowledge_graph_builder()
