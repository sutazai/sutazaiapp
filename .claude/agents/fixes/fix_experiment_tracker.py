#!/usr/bin/env python3
"""
Compliance Fix for experiment-tracker
...
def fix_experiment_tracker():
    """Fix compliance issues for experiment-tracker"""
    
    agent_file = Path('/opt/sutazaiapp/.claude/agents/experiment-tracker.md')
...
def example_agent_action():
    action_desc = "Analyzing codebase for experiment-tracker tasks"
...
- `AGENT_NAME=experiment-tracker` - This agent's name for logging

### Startup Compliance Check:
```bash
# Use the startup wrapper to ensure compliance
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py experiment-tracker
```
'''
    ...
    if modified:
        with open(agent_file, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"✅ Fixed experiment-tracker")
        return True
    else:
        print(f"ℹ️  experiment-tracker already compliant")
        return True

if __name__ == "__main__":
    success = fix_experiment_tracker()
