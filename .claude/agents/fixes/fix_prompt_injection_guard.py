#!/usr/bin/env python3
"""
Compliance Fix for prompt-injection-guard
...
def fix_prompt_injection_guard():
    """Fix compliance issues for prompt-injection-guard"""
    
    agent_file = Path('/opt/sutazaiapp/.claude/agents/prompt-injection-guard.md')
...
def example_agent_action():
    action_desc = "Analyzing codebase for prompt-injection-guard tasks"
...
- `AGENT_NAME=prompt-injection-guard` - This agent's name for logging

### Startup Compliance Check:
```bash
# Use the startup wrapper to ensure compliance
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py prompt-injection-guard
```
'''
    ...
    if modified:
        with open(agent_file, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"✅ Fixed prompt-injection-guard")
        return True
    else:
        print(f"ℹ️  prompt-injection-guard already compliant")
        return True

if __name__ == "__main__":
    success = fix_prompt_injection_guard()
