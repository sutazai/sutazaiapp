#!/usr/bin/env python3
"""
Compliance Fix for edge-inference-proxy-detailed
...
def fix_edge_inference_proxy_detailed():
    """Fix compliance issues for edge-inference-proxy-detailed"""
    
    agent_file = Path('/opt/sutazaiapp/.claude/agents/edge-inference-proxy-detailed.md')
...
environment:
  ...
  - AGENT_NAME=edge-inference-proxy-detailed
  - LOG_LEVEL=INFO
'''
...
def example_agent_action():
    action_desc = "Analyzing codebase for edge-inference-proxy-detailed tasks"
...
- `AGENT_NAME=edge-inference-proxy-detailed` - This agent's name for logging

### Startup Compliance Check:
```bash
# Use the startup wrapper to ensure compliance
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py edge-inference-proxy-detailed
```
'''
    ...
    if modified:
        with open(agent_file, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"✅ Fixed edge-inference-proxy-detailed")
        return True
    else:
        print(f"ℹ️  edge-inference-proxy-detailed already compliant")
        return True

if __name__ == "__main__":
    success = fix_edge_inference_proxy_detailed()
