#!/usr/bin/env python3
"""
Compliance Fix for prompt-injection-guard-detailed
...
def fix_prompt_injection_guard_detailed():
    """Fix compliance issues for prompt-injection-guard-detailed"""
    
    agent_file = Path('/opt/sutazaiapp/.claude/agents/prompt-injection-guard-detailed.md')
...
def example_agent_action():
    action_desc = "Analyzing codebase for prompt-injection-guard-detailed tasks"
...
- `AGENT_NAME=prompt-injection-guard-detailed` - This agent's name for logging
...
'''
    ...
    if modified:
        with open(agent_file, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"✅ Fixed prompt-injection-guard-detailed")
        return True
    else:
        print(f"ℹ️  prompt-injection-guard-detailed already compliant")
        return True

if __name__ == "__main__":
    success = fix_prompt_injection_guard_detailed()
