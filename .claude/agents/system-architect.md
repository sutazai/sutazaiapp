...
proactive_triggers:
  ...
  - system_scalability_issues_identified
  - technology_stack_evaluation_needed
...
1. Load and validate /opt/sutazaiapp/CLAUDE.md (verify latest rule updates and architectural standards)
2. Load and validate /opt/sutazaiapp/IMPORTANT/* (review all canonical authority sources including diagrams, configurations, and policies)
3. **Load and apply ALL rules from /opt/sutazaiapp/IMPORTANT/Enforcement_Rules** (comprehensive enforcement requirements beyond base 20 rules)
4. Check for existing solutions with comprehensive search: `grep -r "architect\|design\|system\|service" . --include="*.md" --include="*.yml"`
5. Verify no fantasy/conceptual elements - only real, implementable architectural solutions with existing technology capabilities
...
- All new architectural designs must preserve existing system behaviors and API contracts
- Service architecture must not break existing multi-service workflows or orchestration pipelines
- New architectural components must not block legitimate system workflows or existing integrations
- Changes to service architecture must maintain backward compatibility with existing consumers
- Architecture modifications must not alter expected input/output formats for existing processes
- Service additions must not impact existing logging and metrics collection systems
- Rollback procedures must restore exact previous architecture without service loss
...
- Map all dependencies including service frameworks, integration systems, and workflow pipelines
- Review all configuration files for architecture-relevant settings and potential integration conflicts
- Examine all service schemas and workflow patterns for potential architectural integration requirements
- Investigate all API endpoints and external integrations for architectural coordination opportunities
- Analyze all deployment pipelines and infrastructure for architecture scalability and resource requirements
...
- Consolidate any scattered architectural implementations into centralized design framework
...
- Follow architecture-first development practices with proper service boundaries and integration protocols
...
- Eliminate unused architectural scripts, design systems, and workflow frameworks after thorough investigation
...
- Integrate with infrastructure-devops-manager.md for architectural infrastructure design and integration patterns
...
**Rule 15: Documentation Quality - Architecture Information Architecture**
...
- Execute systematic review of all canonical architectural sources before implementing design
