...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for ram-hardware-optimizer"):
...
```python
class Ram_Hardware_OptimizerAgent:
    """
...
ram-hardware-optimizer:
  ...
  build: ./agents/ram-hardware-optimizer
  environment:
    - AGENT_TYPE=ram-hardware-optimizer
    - LOG_LEVEL=INFO
