...
name: hardware-resource-optimizer-detailed
...
## Purpose
Specialized AI agent for hardware resource optimizer
...
#!/usr/bin/env python3
"""
Hardware Resource Optimizer - Comprehensive automation platform Agent Implementation
|
 Use this agent when you need to optimize system resources for the SutazAI system, including
 ...
 user: "The system is freezing when running agents"
 ...
 user: "What hardware should I upgrade to improve automation platform performance?"
...
logger = logging.getLogger('hardware_resource_optimizer')
...
class HardwareResourceOptimizer(ComprehensiveSystemInvestigator):
 """
 Hardware Resource Optimizer Implementation
 
 This agent specialized ai agent for hardware resource optimizer
 """
 ...
 logger.info(f"Initialized hardware-resource-optimizer with profile: {self.hardware_profile}")
...
def main():
 ...
 parser = argparse.ArgumentParser(description='Hardware Resource Optimizer')
 ...
 logger.info(f"Starting hardware-resource-optimizer...")
 # Start agent service
...
```yaml
# docker-compose-hardware-resource-optimizer.yml
version: '3.8'
...
services:
 ...
 build:
 context: .
 dockerfile: Dockerfile.hardware-resource-optimizer
 ...
 memory: 4G
 healthcheck:
 test: ["CMD", "python", "-m", "agents.hardware_resource_optimizer", "status"]
 interval: 30s
 timeout: 10s
 retries: 3
```
...
```bash
# Direct execution
python -m agents.hardware_resource_optimizer start

# Docker deployment
docker-compose -f docker-compose-hardware-resource-optimizer.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.hardware_resource_optimizer status
...
```bash
python -m agents.hardware_resource_optimizer test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.hardware_resource_optimizer investigate
```
...
This comprehensive implementation ensures the hardware-resource-optimizer agent operates efficiently within the SutazAI system while maintaining the conservative resource strategy.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for hardware-resource-optimizer-detailed"):
...
- `AGENT_NAME=hardware-resource-optimizer-detailed`

**Startup Check:**
```bash
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py hardware-resource-optimizer-detailed
```
