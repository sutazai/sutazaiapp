...
4. **Domain-Specific Generation**: Text, images, tabular, time series, graphs
...
class SyntheticImageGenerator:
    ...
    def train_stylegan2(self, real_images):
        ...
        for resolution in [4, 8, 16, 32, 64, 128, 256]:
            ...
            resized_data = self.resize_dataset(real_images, resolution)
            ...
            for epoch in range(self.epochs_per_resolution):
                ...
                # Generate images
                fake_images = self.synthesis_network(w, resolution)
                
                # Discriminator predictions
                real_batch = self.sample_batch(resized_data)
                real_pred = self.discriminator(real_batch, resolution)
                fake_pred = self.discriminator(fake_images.detach(), resolution)
                
                # WGAN-GP loss
                d_loss = self.compute_wasserstein_loss(real_pred, fake_pred)
                gp = self.gradient_penalty(real_batch, fake_images, resolution)
                ...
                # Generator training
                fake_pred = self.discriminator(fake_images, resolution)
    ...
    def controlled_generation(self, attributes):
        ...
        for attr_config in attributes:
            ...
            # Generate image
            image = self.synthesis_network(w_edited)
