...
## Purpose
Specialized AI agent for data analysis engineer
...
#!/usr/bin/env python3
"""
Data Analysis Engineer - Comprehensive AGI Agent Implementation
...
logger = logging.getLogger('data_analysis_engineer')
...
class DataAnalysisEngineer(ComprehensiveSystemInvestigator):
    """
    Data Analysis Engineer Implementation
    
    This agent specialized ai agent for data analysis engineer
    """
    ...
    def __init__(self, brain_path: str = "/opt/sutazaiapp/brain"):
        ...
        logger.info(f"Initialized data-analysis-engineer with profile: {self.hardware_profile}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Data Analysis Engineer')
    ...
    # Initialize agent
    agent = DataAnalysisEngineer()
    
    if args.command == 'start':
        logger.info(f"Starting data-analysis-engineer...")
        # Start agent service
...
```yaml
# docker-compose-data-analysis-engineer.yml
version: '3.8'
...
services:
  data-analysis-engineer:
    build:
      context: .
      dockerfile: Dockerfile.data-analysis-engineer
    ...
    deploy:
      resources:
        limits:
          ...
          memory: 4G
    healthcheck:
      test: ["CMD", "python", "-m", "agents.data_analysis_engineer", "status"]
      interval: 30s
      timeout: 10s
      retries: 3
```
...
```bash
# Direct execution
python -m agents.data_analysis_engineer start

# Docker deployment
docker-compose -f docker-compose-data-analysis-engineer.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.data_analysis_engineer status
...
```bash
python -m agents.data_analysis_engineer test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.data_analysis_engineer investigate
```
...
This comprehensive implementation ensures the data-analysis-engineer agent operates efficiently within the SutazAI advanced AI system while maintaining the conservative resource strategy.