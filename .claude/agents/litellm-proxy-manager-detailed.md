...
## Purpose
Specialized AI agent for litellm proxy manager
...
#!/usr/bin/env python3
"""
Litellm Proxy Manager - Comprehensive AGI Agent Implementation
...
logger = logging.getLogger('litellm_proxy_manager')
...
class LitellmProxyManager(ComprehensiveSystemInvestigator):
    """
    Litellm Proxy Manager Implementation
    
    This agent specialized ai agent for litellm proxy manager
    """
    ...
    def __init__(self, brain_path: str = "/opt/sutazaiapp/brain"):
        ...
        logger.info(f"Initialized litellm-proxy-manager with profile: {self.hardware_profile}")
...
def main():
    ...
    # Initialize agent
    agent = LitellmProxyManager()
    
    if args.command == 'start':
        logger.info(f"Starting litellm-proxy-manager...")
        # Start agent service
...
```yaml
# docker-compose-litellm-proxy-manager.yml
version: '3.8'
...
services:
  litellm-proxy-manager:
    build:
      context: .
      dockerfile: Dockerfile.litellm-proxy-manager
    ...
    deploy:
      resources:
        limits:
          ...
          memory: 4G
    healthcheck:
      test: ["CMD", "python", "-m", "agents.litellm_proxy_manager", "status"]
      interval: 30s
      timeout: 10s
      retries: 3
```
...
```bash
# Direct execution
python -m agents.litellm_proxy_manager start

# Docker deployment
docker-compose -f docker-compose-litellm-proxy-manager.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.litellm_proxy_manager status
...
```bash
python -m agents.litellm_proxy_manager test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.litellm_proxy_manager investigate
```
...
This comprehensive implementation ensures the litellm-proxy-manager agent operates efficiently within the SutazAI advanced AI system while maintaining the conservative resource strategy.