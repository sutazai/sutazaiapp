...
name: financial-analysis-specialist-detailed
...
## Purpose
Specialized AI agent for financial analysis specialist
...
#!/usr/bin/env python3
"""
Financial Analysis Specialist - Comprehensive automation platform Agent Implementation
...
logger = logging.getLogger('financial_analysis_specialist')
...
class FinancialAnalysisSpecialist(ComprehensiveSystemInvestigator):
 """
 Financial Analysis Specialist Implementation
 
 This agent specialized ai agent for financial analysis specialist
 """
 ...
 logger.info(f"Initialized financial-analysis-specialist with profile: {self.hardware_profile}")
...
def main():
 ...
 parser = argparse.ArgumentParser(description='Financial Analysis Specialist')
 ...
 # Initialize agent
 agent = FinancialAnalysisSpecialist()
 
 if args.command == 'start':
 logger.info(f"Starting financial-analysis-specialist...")
 # Start agent service
...
```yaml
# docker-compose-financial-analysis-specialist.yml
version: '3.8'
...
services:
 ...
 build:
 context: .
 dockerfile: Dockerfile.financial-analysis-specialist
 ...
 memory: 4G
 healthcheck:
 test: ["CMD", "python", "-m", "agents.financial_analysis_specialist", "status"]
 interval: 30s
 timeout: 10s
 retries: 3
```
...
```bash
# Direct execution
python -m agents.financial_analysis_specialist start

# Docker deployment
docker-compose -f docker-compose-financial-analysis-specialist.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.financial_analysis_specialist status
...
```bash
python -m agents.financial_analysis_specialist test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.financial_analysis_specialist investigate
```
...
This comprehensive implementation ensures the financial-analysis-specialist agent operates efficiently within the SutazAI system while maintaining the conservative resource strategy.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for financial-analysis-specialist-detailed"):
...
- `AGENT_NAME=financial-analysis-specialist-detailed`

**Startup Check:**
```bash
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py financial-analysis-specialist-detailed
```
