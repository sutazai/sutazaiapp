...
name: agentzero-coordinator-detailed
...
## Purpose
Specialized AI agent for agentzero coordinator
...
#!/usr/bin/env python3
"""
Agentzero Coordinator - Comprehensive automation platform Agent Implementation
...
logger = logging.getLogger('agentzero_coordinator')
...
class AgentzeroCoordinator(ComprehensiveSystemInvestigator):
 """
 Agentzero Coordinator Implementation
 
 This agent specialized ai agent for agentzero coordinator
 """
 ...
 logger.info(f"Initialized agentzero-coordinator with profile: {self.hardware_profile}")
...
def main():
 ...
 parser = argparse.ArgumentParser(description='Agentzero Coordinator')
 ...
 # Initialize agent
 agent = AgentzeroCoordinator()
 
 if args.command == 'start':
 logger.info(f"Starting agentzero-coordinator...")
 # Start agent service
...
```yaml
# docker-compose-agentzero-coordinator.yml
version: '3.8'
...
services:
 ...
 build:
 context: .
 dockerfile: Dockerfile.agentzero-coordinator
 ...
 memory: 4G
 healthcheck:
 test: ["CMD", "python", "-m", "agents.agentzero_coordinator", "status"]
 interval: 30s
 timeout: 10s
 retries: 3
```
...
```bash
# Direct execution
python -m agents.agentzero_coordinator start

# Docker deployment
docker-compose -f docker-compose-agentzero-coordinator.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.agentzero_coordinator status
...
```bash
python -m agents.agentzero_coordinator test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.agentzero_coordinator investigate
```
...
This comprehensive implementation ensures the agentzero-coordinator agent operates efficiently within the SutazAI system while maintaining the conservative resource strategy.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for agentzero-coordinator-detailed"):
...
- `AGENT_NAME=agentzero-coordinator-detailed`

**Startup Check:**
```bash
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py agentzero-coordinator-detailed
```
