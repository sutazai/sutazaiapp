...
name: private-data-analyst-detailed
...
## Purpose
Specialized AI agent for private data analyst
...
#!/usr/bin/env python3
"""
Private Data Analyst - Comprehensive automation platform Agent Implementation
...
logger = logging.getLogger('private_data_analyst')
...
class PrivateDataAnalyst(ComprehensiveSystemInvestigator):
 """
 Private Data Analyst Implementation
 
 This agent specialized ai agent for private data analyst
 """
 ...
 logger.info(f"Initialized private-data-analyst with profile: {self.hardware_profile}")
...
def main():
 ...
 parser = argparse.ArgumentParser(description='Private Data Analyst')
 ...
 # Initialize agent
 agent = PrivateDataAnalyst()
 
 if args.command == 'start':
 logger.info(f"Starting private-data-analyst...")
 # Start agent service
...
```yaml
# docker-compose-private-data-analyst.yml
version: '3.8'
...
services:
 ...
 build:
 context: .
 dockerfile: Dockerfile.private-data-analyst
 ...
 memory: 4G
 healthcheck:
 test: ["CMD", "python", "-m", "agents.private_data_analyst", "status"]
 interval: 30s
 timeout: 10s
 retries: 3
```
...
```bash
# Direct execution
python -m agents.private_data_analyst start

# Docker deployment
docker-compose -f docker-compose-private-data-analyst.yml up -d
```

### Example 2: Checking Agent Status
```bash
python -m agents.private_data_analyst status
...
```bash
python -m agents.private_data_analyst test --task '{"type": "analyze", "data": "test"}'
```

### Example 4: System Investigation
```bash
python -m agents.private_data_analyst investigate
```
...
This comprehensive implementation ensures the private-data-analyst agent operates efficiently within the SutazAI system while maintaining the conservative resource strategy.
...
# Example usage
def example_task():
    if safe_execute_action("Analyzing codebase for private-data-analyst-detailed"):
...
- `AGENT_NAME=private-data-analyst-detailed`

**Startup Check:**
```bash
python3 /opt/sutazaiapp/.claude/agents/agent_startup_wrapper.py private-data-analyst-detailed
```
