...
def get_container_stats():
    ...
    try:
        cmd = ['docker', 'stats', 'sutazai-neo4j', '--no-stream', '--format', 'table {{.Container}}\t{{.CPUPerc}}\t{{.MemUsage}}\t{{.MemPerc}}\t{{.NetIO}}\t{{.BlockIO}}']
...
def get_database_metrics():
    ...
    try:
        ...
        cmd = ['docker', 'exec', 'sutazai-neo4j', 'cypher-shell', '-u', 'neo4j', '-p', password, "CALL dbms.listConfig() YIELD name, value WHERE name CONTAINS 'memory' RETURN name, value LIMIT 10"]
...
def get_heap_dump_analysis():
    ...
    try:
        cmd = ['docker', 'exec', 'sutazai-neo4j', 'sh', '-c', "jstat -gc -t $(pgrep -f 'org.neo4j') 2>/dev/null | tail -1"]
        result = subprocess.run(cmd, capture_output=True, text=True, check=True)
