...
class LoggingStandardizer:
    ...
    def __init__(self, root_dir: str):
        ...
        self.logger_types: Dict[str, str] = {}  # file -> logger type (logging/loguru)
        
    def find_python_files(self) -> None:
        """Find all Python files in the project."""
        for root, _, files in os.walk(self.root_dir):
            # Skip virtual environments, .git, etc.
            if any(part in ['.venv', 'venv', '.git', '.pytest_cache', '__pycache__', '.mypy_cache']
    ...
    def analyze_logging_usage(self) -> None:
        ...
        print(f"  - Standard logging: {sum(1 for v in self.logger_types.values() if v == 'logging')}")
        print(f"  - Loguru: {sum(1 for v in self.logger_types.values() if v == 'loguru')}")
            ...
            try:
                ...
                if 'import os' not in content and logger_type:
                    ...
                    if import_section_end > 0:
                        ...
                        content = content[:import_section_end] + '\nimport os' + content[import_section_end:]
                
                # Replace logging configurations
                for pattern, replacement in LOGGING_CONFIG_PATTERNS:
                    if logger_type == 'logging' and 'logging.basicConfig' in pattern.pattern:
                        content = pattern.sub(replacement, content)
                    elif logger_type == 'loguru' and 'logger.add' in pattern.pattern:
                        content = pattern.sub(replacement, content)
                ...
                if content != original_content:
                    ...
                    print(f"✅ Updated logging in {os.path.relpath(file_path, self.root_dir)}")
