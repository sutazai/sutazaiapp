...
def show_hardware_optimization():
    ...
    try:
        ...
        if direct_status:
            ...
            hardware_metrics = {'cpu_percent': np.random.uniform(10, 80), 'memory_percent': np.random.uniform(30, 90), 'disk_percent': np.random.uniform(5, 50), 'memory_available_gb': np.random.uniform(8, 32), 'disk_free_gb': np.random.uniform(100, 1000), 'timestamp': time.time()}
    except Exception as e:
        st.error(f'❌ API Integration Error: {str(e)}')
        hardware_metrics = {'cpu_percent': 0, 'memory_percent': 0, 'disk_percent': 0, 'memory_available_gb': 0, 'disk_free_gb': 0, 'timestamp': time.time()}
    ...
    with metrics_col1:
        ...
        st.metric('CPU Usage', f'{cpu_percent:.1f}%', delta=f'{('Normal' if cpu_percent < 50 else 'High' if cpu_percent < 80 else 'Critical')}')
        fig_cpu = go.Figure(go.Indicator(mode='gauge+number', value=cpu_percent, domain={'x': [0, 1], 'y': [0, 1]}, title={'text': 'CPU %'}, gauge={'axis': {'range': [None, 100]}, 'bar': {'color': 'darkblue'}, 'steps': [{'range': [0, 50], 'color': 'lightgray'}, {'range': [50, 80], 'color': 'yellow'}, {'range': [80, 100], 'color': 'red'}], 'threshold': {'line': {'color': 'red', 'width': 4}, 'thickness': 0.75, 'value': 90}}))
    ...
    with metrics_col2:
        ...
        st.metric('Memory Usage', f'{memory_percent:.1f}%', delta=f'{memory_available:.1f}GB free')
        fig_mem = go.Figure(go.Indicator(mode='gauge+number', value=memory_percent, domain={'x': [0, 1], 'y': [0, 1]}, title={'text': 'Memory %'}, gauge={'axis': {'range': [None, 100]}, 'bar': {'color': 'darkgreen'}, 'steps': [{'range': [0, 60], 'color': 'lightgray'}, {'range': [60, 85], 'color': 'yellow'}, {'range': [85, 100], 'color': 'red'}], 'threshold': {'line': {'color': 'red', 'width': 4}, 'thickness': 0.75, 'value': 95}}))
    ...
    with metrics_col3:
        ...
        st.metric('Disk Usage', f'{disk_percent:.1f}%', delta=f'{disk_free:.1f}GB free')
        fig_disk = go.Figure(go.Indicator(mode='gauge+number', value=disk_percent, domain={'x': [0, 1], 'y': [0, 1]}, title={'text': 'Disk %'}, gauge={'axis': {'range': [None, 100]}, 'bar': {'color': 'darkorange'}, 'steps': [{'range': [0, 70], 'color': 'lightgray'}, {'range': [70, 90], 'color': 'yellow'}, {'range': [90, 100], 'color': 'red'}], 'threshold': {'line': {'color': 'red', 'width': 4}, 'thickness': 0.75, 'value': 95}}))
    ...
    st.session_state.perf_history.append({'timestamp': current_time, 'cpu': cpu_percent, 'memory': memory_percent, 'disk': disk_percent})
    ...
    if len(st.session_state.perf_history) >= 2:
        ...
        fig_trends.add_trace(go.Scatter(x=df['timestamp'], y=df['cpu'], mode='lines+markers', name='CPU %', line=dict(color='blue', width=2)))
        fig_trends.add_trace(go.Scatter(x=df['timestamp'], y=df['memory'], mode='lines+markers', name='Memory %', line=dict(color='green', width=2)))
        fig_trends.add_trace(go.Scatter(x=df['timestamp'], y=df['disk'], mode='lines+markers', name='Disk %', line=dict(color='orange', width=2)))
        fig_trends.update_layout(title='System Performance Trends', xaxis_title='Time', yaxis_title='Usage %', height=400, showlegend=True)
    ...
    with control_col2:
        ...
        performance_mode = st.selectbox('Performance Mode:', ['Balanced', 'Performance', 'Power Saving', 'High Performance'], index=0)
        if st.button('🚀 Apply Performance Mode', use_container_width=True):
            with st.spinner(f'Applying {performance_mode} mode...'):
                try:
                    ...
                    perf_result = sync_call_api('http://127.0.0.1:11110/optimize/performance', method='POST', data=perf_data, timeout=10.0)
                    if perf_result:
    ...
    with control_col3:
        ...
        if st.button('📊 Generate Report', use_container_width=True):
            report_data = {'timestamp': datetime.now().isoformat(), 'system_metrics': hardware_metrics, 'performance_history': st.session_state.perf_history[-10:], 'optimization_history': st.session_state.optimization_history}
            st.download_button('📥 Download System Report', json.dumps(report_data, indent=2), file_name=f'hardware_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json', mime='application/json')
    ...
    with st.expander('🔍 Advanced Diagnostics', expanded=False):
        ...
        with diag_col1:
            ...
            endpoints = [{'name': 'Backend API', 'url': 'http://127.0.0.1:10010/health'}, {'name': 'Hardware Optimizer', 'url': 'http://127.0.0.1:11110/health'}, {'name': 'Ollama AI', 'url': 'http://127.0.0.1:10104/api/tags'}, {'name': 'Database', 'url': 'http://127.0.0.1:10000/health'}, {'name': 'Redis', 'url': 'http://127.0.0.1:10001/health'}]
        ...
        with test_col2:
            if st.button('📋 Test Invalid Data'):
                try:
                    result = sync_call_api('http://127.0.0.1:11110/optimize/invalid', method='POST', data={'invalid': 'data'}, timeout=2.0)
