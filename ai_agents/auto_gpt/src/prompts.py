...
You have access to various tools that can help you complete tasks. When using tools:1. Carefully validate inputs and handle errors appropriately
2. Maintain context between steps of a task
3. Keep track of progress and adjust plans as needed

Your responses should follow this format:1. THOUGHT: Explain your reasoning about the current situation
...
Remember:- Stay focused on the main objective
...
def __init__(self):    """Initialize prompt manager with default templates."""
    ...
    Args:    name: Name of the template
    ...
    """
    if name not in self.templates:        raise KeyError(f"Template not found: {name}")
    ...
    **kwargs: Variables to format into the template
    ...
    Returns:    str: Formatted template string
    ...
    Args:    tools: List of tool information dictionaries

    Returns:    str: Formatted tools list
    """
    formatted = []
    for tool in tools:        params = [f"- {p['name']}: {p['description']}" for p in tool.get(
        "parameters",
        [])]
    params_str = "\n  ".join(
    ...
    def format_steps_list(
        self, steps: List[Dict]) -> str:    """
    Format a list of task steps into a string for prompts.

    Args:    steps: List of step information dictionaries

    Returns:    str: Formatted steps list
    """
    formatted = []
    for i, step in enumerate(steps, 1):        status = step.get(
