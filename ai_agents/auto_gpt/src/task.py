...
class Task:
    ...
    def status(self) -> TaskStatus:
        ...
        if any(step.status == TaskStatus.IN_PROGRESS for step in self.steps):
    ...
    def add_step(self, description: str) -> TaskStep:
        ...
        if len(self.steps) >= self.max_steps:
            raise ValueError(f"Cannot add step: maximum of {self.max_steps} steps allowed")
    ...
    def complete_step(self, index: int, result: Optional[str] = None) -> None:
        """
        ...
        if all(s.status == TaskStatus.COMPLETED for s in self.steps):
    ...
    def load(self) -> None:
        """Load task state from disk if persist_path is set."""
        ...
        self.steps = [TaskStep.from_dict(step) for step in data["steps"]]
        self.created_at = datetime.fromisoformat(data["created_at"])
        self.started_at = datetime.fromisoformat(data["started_at"]) if data["started_at"] else None
        self.completed_at = datetime.fromisoformat(data["completed_at"]) if data["completed_at"] else None
