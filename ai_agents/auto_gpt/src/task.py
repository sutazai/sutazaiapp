...
class TaskStatus(Enum):    """Enumeration of possible task statuses."""
...
PENDING = "pending"
    ...
    @dataclass
    class TaskStep:        """Represents a single step in a task's execution plan."""
    ...
    def to_dict(self) -> Dict:    """Convert task step to dictionary format."""
    return {
    ...
    def from_dict(
    cls,
    data: Dict) -> "TaskStep":    """Create task step from dictionary format."""
    return cls(
    ...
    class Task:        """Represents a task to be executed by the agent."""
    ...
    Args:    objective: The main objective of the task
    ...
    @property
    def status(self) -> TaskStatus:    """Get the current status of the task."""
    if not self.steps:        return TaskStatus.PENDING

    if any(
            step.status == TaskStatus.FAILED for step in self.steps):                return TaskStatus.FAILED

    if all(
            step.status == TaskStatus.COMPLETED for step in self.steps):                return TaskStatus.COMPLETED

    if any(
            step.status == TaskStatus.IN_PROGRESS for step in self.steps):                return TaskStatus.IN_PROGRESS
    ...
    Args:    description: Description of the step

    Returns:    TaskStep: The created step

    Raises:    ValueError: If maximum number of steps would be exceeded
    """
    if len(self.steps) >= self.max_steps:        raise ValueError(
    ...
    Args:    index: Index of the step to start

    Raises:    IndexError: If index is out of range
    ValueError: If step is not in PENDING status
    """
    if not 0 <= index < len(self.steps):        raise IndexError(
    ...
    def complete_step(
        self, index: int, result: Optional[str] = None) -> None:    """
    Mark a step as completed.

    Args:    index: Index of the step to complete
    ...
    Args:    index: Index of the step to fail
    error: Error message explaining the failure

    Raises:    IndexError: If index is \
    out of range
    ValueError: If step is not in \
    IN_PROGRESS status
    """
    ...
    def to_dict(
        self) -> Dict:    """Convert task to dictionary format."""
    return {
    ...
    def save(
        self) -> None:    """Save task state to disk if persist_path is \
    set."""
    ...
    def load(
        self) -> None:    """Load task state from disk if persist_path is \
    set."""
    if not self.persist_path or not os.path.exists(
            self.persist_path):                return

    with open(
        self.persist_path,
            "r") as f:                data = json.load(
        f)
