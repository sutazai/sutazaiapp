...
class AutoGPTAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.memory = Memory(max_messages=self.config.memory_size, persist_path=os.path.join(log_dir, "memory.json"))
    ...
    async def shutdown(self) -> None:
        ...
        try:
            ...
            logger.info(f"Shut down {self.name} agent")

        except Exception as e:
            raise AgentError(f"Failed to shut down agent: {str(e)}")
    ...
    async def execute(self, task: Union[str, Dict]) -> Dict:
        ...
        Args:
            task: Task description or configuration dictionary
        ...
        try:
            ...
            if isinstance(task, str):
                self.current_task = Task(
                    ...
                    persist_path=os.path.join(self.log_dir, "task.json"),
                )
            else:
                self.current_task = Task(
                    objective=task["objective"],
                    context=task.get("context"),
                    max_steps=task.get("max_steps", self.config.max_iterations),
                    persist_path=os.path.join(self.log_dir, "task.json"),
                )
            ...
            iteration = 0
            while (
                self.current_task.status not in (TaskStatus.COMPLETED, TaskStatus.FAILED)
    ...
    async def _plan_task(self) -> List[str]:
        """
        ...
        try:
            # Format tools list
            tools_list = self.prompts.format_tools_list(registry.list_tools())
            ...
            prompt = self.prompts.format_template(
                ...
                context=(
                    json.dumps(self.current_task.context, indent=2)
            ...
            response = await self.model.get_response(
                system_prompt=self.prompts.get_template("system"), functions=registry.list_tools()
            )
            ...
            raise AgentError(f"Failed to create task plan: {str(e)}")
    ...
    async def _execute_step(self) -> Optional[str]:
        ...
        try:
            ...
            previous_steps = self.prompts.format_steps_list([s.to_dict() for s in self.current_task.steps[:step_index]])
            ...
            self.current_task.start_step(step_index)
            ...
            if isinstance(response, dict):
                tool = registry.get_tool(response["function"])
                result = tool.execute(response["arguments"])
                self.current_task.complete_step(step_index, str(result))
                return str(result)

            # Handle text response
            self.current_task.complete_step(step_index, response)
            return response

        except Exception as e:
            if next_step:
                self.current_task.fail_step(step_index, str(e))
            raise AgentError(f"Failed to execute step: {str(e)}")

    async def _evaluate_progress(self) -> None:
        """
        Evaluate task progress and adjust plans if needed.
        ...
        try:
            ...
            for step in self.current_task.steps:
                if step.status == TaskStatus.COMPLETED:
                    ...
                    remaining.append(step.to_dict())

            # Format steps lists
            completed_steps = self.prompts.format_steps_list(completed)
            remaining_steps = self.prompts.format_steps_list(remaining)

            # Get evaluation prompt
            prompt = self.prompts.format_template(
                "task_evaluation",
                objective=self.current_task.objective,
                completed_steps=completed_steps or "No steps completed",
                results="See individual step results above",
                remaining_steps=remaining_steps or "No remaining steps",
            )

            # Get model response
            response = await self.model.get_response(system_prompt=self.prompts.get_template("system"))

            # TODO: Parse response and adjust plan if needed

        except Exception as e:
            raise AgentError(f"Failed to evaluate progress: {str(e)}")
    ...
    async def _handle_error(self, error_message: str) -> None:
        ...
        try:
            ...
            raise AgentError(f"Failed to handle error: {str(e)}")
    ...
    def register_tool(self, tool: Tool) -> None:
        ...
        try:
            ...
            raise AgentError(f"Failed to register tool: {str(e)}")
    ...
    def get_tool(self, name: str) -> Tool:
        ...
        Raises:
            AgentError: If tool is not found
        """
        try:
            return registry.get_tool(name)
        except Exception as e:
            raise AgentError(f"Failed to get tool: {str(e)}")
    ...
    def list_tools(self) -> List[Dict]:
        ...
        try:
            ...
            raise AgentError(f"Failed to list tools: {str(e)}")
