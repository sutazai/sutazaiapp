...
@pytest.fixture
    ...
    Returns:    str: Path to the sample PDF
    """
    import fitz  # type: ignore

    # Create a temporary PDF
    with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as temp_pdf:        doc = fitz.open()
    ...
    Returns:    str: Path to the sample image
    """
    # Create a temporary image
    with tempfile.NamedTemporaryFile(
        suffix=".png", delete=False
    ) as temp_img:        image = np.zeros((200, 200), dtype=np.uint8)
    ...
    with pytest.raises(AgentError) as excinfo:        agent.execute(task)

    assert "Document processing failed" in str(
        excinfo.value
    )
    ...
    def test_document_utils_validation(
        self, sample_pdf_path: str
    ):    """
    Test document validation utility
    """
    validation_result = (
        DocumentUtils.validate_document(
            sample_pdf_path
        )
    )
    ...
    def test_document_utils_metadata(
        self, sample_pdf_path: str
    ):    """
    ...
    """
    test_text = "This is a test document in \
    English."
    languages = DocumentUtils.detect_document_language(
        test_text
    )
    ...
    pytest.main(
        [
            ...
            "--cov=ai_agents.document_processor",  # Coverage for document processor
