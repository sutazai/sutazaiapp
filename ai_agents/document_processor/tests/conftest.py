#!/usr/bin/env python3.11import osimport sysimport pytestfrom loguru import loggerdef pytest_configure(config):    """
...
def pytest_addoption(parser):        """
    ...
    @pytest.fixture(scope="function")        def temp_document_dir(test_environment):            """
    ...
    def pytest_terminal_summary(terminalreporter, exitstatus, config):                """
        ...
        logger.info(
        "Total Tests: %s",
        terminalreporter.stats.get("call", []),
        )
        logger.info(
        "Passed: %s",
