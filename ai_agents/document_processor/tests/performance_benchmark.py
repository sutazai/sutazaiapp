...
@staticmethod
    ...
    Args:    num_docs (int): Number of test documents to generate

    Returns:    list: Paths to generated test documents
    """
    test_docs = []

    for i in range(num_docs):        # PDF document
    with tempfile.NamedTemporaryFile(
        suffix=".pdf",
            delete=False) as temp_pdf:                doc = fitz.open()
    # Updated for PyMuPDF compatibility with Python 3.11
    page = doc.new_page(width=595, height=842)  # A4 size
    page.insert_text(
        (50, 50),
        f"SutazAI Performance Test Document {i}")
    ...
    with tempfile.NamedTemporaryFile(
        suffix=".png",
            delete=False) as temp_img:                image = np.zeros((200, 200), dtype=np.uint8)
    ...
    Args:    documents (list): List of document paths

    Returns:    Dict: Performance benchmark results
    """
    agent = DocumentProcessorAgent()
    extraction_times = []

    for doc_path in documents:        if not doc_path.endswith(".pdf"):    continue
    ...
    return {
        ...
        "mean_time": statistics.mean(
            extraction_times) if extraction_times else 0,
        "median_time": (
            statistics.median(extraction_times) if extraction_times else 0),
    ...
    for doc_path in documents:        if not doc_path.endswith(".png"):    continue
    ...
    return {
        ...
        "mean_time": statistics.mean(
            analysis_times) if analysis_times else 0,
        "median_time": statistics.median(
            analysis_times) if analysis_times else 0,
    ...
    Args:    num_docs (int): Number of test documents

    Returns:    Dict: Comprehensive benchmark results
    """
    ...
    for doc in test_documents:        os.unlink(doc)
    ...
    for task, results in benchmark_results.items():        assert results["mean_time"] < 5.0, f"{task} performance too slow"
    assert results[
        "total_documents"] >= 0, f"No documents processed for {task}"
    ...
    with open(
        report_path,
        "w",
            encoding="utf-8") as report_file:                json.dump(
        benchmark_results,
        report_file,
        indent=2)
