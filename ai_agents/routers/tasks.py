...
async def submit_task(
    ...
    """
    try:
        if request.agent_id not in manager.agents:
            raise HTTPException(
                status_code=404, detail=f"Agent {request.agent_id} not found"
            )

        agent = manager.agents[request.agent_id]
        ...
        background_tasks.add_task(
            manager.execute_task, task_id, request.parameters or {}, request.timeout
        )
...
async def get_task_status(
    task_id: str, manager: AgentManager = Depends(get_agent_manager)
) -> TaskResponse:
    """
    ...
    """
    try:
        if task_id not in manager.task_queue:
            raise HTTPException(status_code=404, detail=f"Task {task_id} not found")

        return manager.task_queue[task_id]["task"]
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_agent_tasks(
    ...
    try:
        ...
        tasks = [
            task_data["task"]
            for task_data in manager.task_queue.values()
...
async def cancel_task(
    ...
    try:
        ...
        task_data = manager.task_queue[task_id]
        task = task_data["task"]
...
async def get_queue_status(
    ...
    try:
        return {
            "total_tasks": len(manager.task_queue),
            "pending_tasks": len(
                [
                    task
                    for task in manager.task_queue.values()
...
async def clear_task_queue(
    status: Optional[str] = None, manager: AgentManager = Depends(get_agent_manager)
) -> Dict[str, str]:
    """
    ...
    try:
        if status:
            ...
            tasks_to_remove = [
                task_id
                for task_id, task_data in manager.task_queue.items()
                if task_data["task"].status == status
            ]
            for task_id in tasks_to_remove:
                del manager.task_queue[task_id]
        else:
            # Clear all tasks
