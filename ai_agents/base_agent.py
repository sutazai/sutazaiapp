...
class BaseAgent(ABC):    """
    ...
    def __init__(self, name: Optional[str] = None,
        ...
        Args:            name: Optional name for the agent
    ...
    def execute(self, task: Dict[str, Any]) -> Dict[str, Any]:        """
        ...
        Args:            task: A dictionary describing the task to be executed

        Returns:            Dict containing task execution results
        """
        pass
    ...
    def __repr__(self) -> str:        return f"{self.__class__.__name__}(id={self.id}, name={self.name})"
...
class BaseAgentImplementation:    """Base implementation class providing common agent functionality."""
    ...
    def _log_performance(
            self, task: Dict[str, Any], result: Dict[str, Any]) -> None:        """
        Log agent performance for each task

        Args:            task (Dict): Executed task details
    ...
    def _persist_performance_history(
            self, max_records: int = 100) -> None:        """
        Persist performance history to a file, maintaining a maximum number of records

        Args:            max_records (int): Maximum number of performance records to keep
        """
        try:            history_file = os.path.join(
                self.log_dir, f"{self.agent_name}_performance_history.json"
            )

            # Trim history if exceeding max records
            trimmed_history = self.performance_history[-max_records:]

            with open(history_file, "w", encoding="utf-8") as f:                json.dump(trimmed_history, f, indent=2, default=str)

        except (IOError, json.JSONDecodeError) as e:            logger.error("Failed to persist performance history: %s", e)

    def get_performance_summary(self) -> Dict[str, Any]:        """
        Get a summary of agent performance

        Returns:            Dict containing performance metrics
        """
        total_tasks = len(self.performance_history)
        if total_tasks == 0:            return {"total_tasks": 0, "success_rate": 0.0}

        successful_tasks = sum(
            1 for record in self.performance_history if record.get(
                "success", False))
