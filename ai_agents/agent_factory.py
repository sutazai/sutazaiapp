...
class AgentFactory:    """
    ...
    def __init__(
        ...
        Args:            agents_dir (str): Base directory for AI agents
    ...
    def _discover_agents(self):        """
        ...
        for agent_type in os.listdir(self.agents_dir):            agent_path = os.path.join(self.agents_dir, agent_type)

            # Skip if not a directory or if it is the base_agent which is
            # abstract
            if not os.path.isdir(agent_path) or agent_type == "base_agent":                continue

            try:                module = importlib.import_module(f"ai_agents.{agent_type}.src")
                agent_class = getattr(
                    module,
                ...
                if agent_class and issubclass(agent_class, BaseAgent):                    self._agent_registry[agent_type] = agent_class
                    logger.info("✅ Registered Agent: %s", agent_type)
            except Exception as e:                logger.exception(
                    f"❌ Failed to register agent {agent_type}: {e}"
                )
    ...
    def create_agent(
        ...
        Args:            agent_type (str): Type of agent to create
            config (Optional[Dict]): Agent configuration

        Returns:            BaseAgent: Instantiated agent

        Raises:            ValueError: If agent type is not registered
        """
        if agent_type not in self._agent_registry:            raise ValueError(
                f"Unknown agent type: {agent_type}. Available types: {', '.join(self._agent_registry.keys())}"
            )

        # Load configuration if not provided
        if config is None:            try:                config = self.config_manager.load_config(agent_type)
    ...
    def list_available_agents(
        ...
        Returns:            Dict: Mapping of agent types to their class names
        """
        return {
            agent_type: agent_class.__name__ for agent_type,
            agent_class in self._agent_registry.items()}

    def main():        """Demonstration of Agent Factory capabilities"""
        factory = AgentFactory()

        # List available agents
        print(
            "Available Agents:", factory.list_available_agents())

        # Create an agent
        try:            auto_gpt_agent = factory.create_agent("auto_gpt")
            print(
                f"Agent Performance: {auto_gpt_agent.get_performance_summary()}")
        except Exception as e:            logger.exception(
