...
class AgentFactory:
    ...
    def __init__(
        ...
        def _discover_agents(self):
                        """Discover and \
                register available agent classes by scanning the agents directory."""
                    ...
                    if agent_class and issubclass(agent_class, BaseAgent):
                        ...
                        logger.info("✅ Registered Agent: %s", agent_type)
                            ...
                            def create_agent(
                                ...
                                raise ValueError(
                                f"Unknown agent type: {agent_type}. Available types: {', '.join(
                                    self._agent_registry.keys())}"
                                )
                                ...
                                if config is None:
                                    try:
                                        config = self.config_manager.load_config(
                                            ...
                                            logger.warning(
                                                "Using default configuration for %s: {e}",
                                                agent_type)
                                            config = {}
                                        ...
                                        def list_available_agents(
                                            ...
                                            """
                                        return {agent_type: agent_class.__name__ for agent_type, agent_class in self._agent_registry.items()}
                                        ...
                                        def main():
                                            ...
                                            # Create an agent
                                            try:
                                                auto_gpt_agent = factory.create_agent(
                                                    "auto_gpt")
                                                print(
