#!/usr/bin/env python3
"""
FastAPI main application for ai-system-architect agent
...
from app import Ai_System_ArchitectAgent
...
agent_instance: Ai_System_ArchitectAgent = None
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting ai-system-architect agent...")
    agent_instance = Ai_System_ArchitectAgent()
    logger.info(f"Agent {agent_instance.name} initialized on port {agent_instance.port}")
    
    yield
    
    # Shutdown
    logger.info("Shutting down ai-system-architect agent...")
    if agent_instance:
...
app = FastAPI(
    ...
    description="SutazAI AI System Architect Agent API",
    version="1.0.0",
    lifespan=lifespan
)
...
async def root():
    """Root endpoint"""
    return {
        "message": "AI System Architect Agent API",
...
async def process_task(task: TaskRequest):
    ...
    try:
        ...
        return TaskResponse(
            status="error",
            error=str(e),
            agent=agent_instance.agent_id if agent_instance else "ai-system-architect"
        )
