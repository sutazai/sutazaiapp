#!/usr/bin/env python3
"""
FastAPI main application for ai-qa-team-lead agent
...
from app import Ai_Qa_Team_LeadAgent
...
agent_instance: Ai_Qa_Team_LeadAgent = None
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting ai-qa-team-lead agent...")
    agent_instance = Ai_Qa_Team_LeadAgent()
    logger.info(f"Agent {agent_instance.name} initialized on port {agent_instance.port}")
    
    yield
    
    # Shutdown
    logger.info("Shutting down ai-qa-team-lead agent...")
    if agent_instance:
        agent_instance.status = "shutdown"

# Create FastAPI app
app = FastAPI(
    title="AI QA Team Lead Agent",
    description="SutazAI AI QA Team Lead Agent API",
    version="1.0.0",
    lifespan=lifespan
)
...
async def root():
    """Root endpoint"""
    return {
        "message": "AI QA Team Lead Agent API",
...
async def process_task(task: TaskRequest):
    ...
    try:
        ...
        return TaskResponse(
            status="error",
            error=str(e),
            agent=agent_instance.agent_id if agent_instance else "ai-qa-team-lead"
        )
