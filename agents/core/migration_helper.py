...
class LegacyAgentWrapper(BaseAgent):
    ...
    def __init__(self, legacy_agent_class: Type[BaseAgentV1], *args, **kwargs):
        """
    ...
    def _extract_legacy_methods(self):
        ...
        for name, method in inspect.getmembers(self.legacy_agent_class, inspect.isfunction):
            ...
            if not name.startswith('_') and hasattr(BaseAgentV1, name):
                # Check if method is overridden
                legacy_method = getattr(self.legacy_agent_class, name)
                base_method = getattr(BaseAgentV1, name)
    ...
    try:
        ...
        try:
            ...
            if issubclass(agent_class, BaseAgentV1) and not issubclass(agent_class, BaseAgent):
                ...
                logger.info(f"Creating v2 agent for {agent_name}")
                return agent_class()
