#!/usr/bin/env python3
"""
FastAPI main application for agent-debugger agent
...
from app import Agent_DebuggerAgent
...
agent_instance: Agent_DebuggerAgent = None
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting agent-debugger agent...")
    agent_instance = Agent_DebuggerAgent()
    logger.info(f"Agent {agent_instance.name} initialized on port {agent_instance.port}")
    
    yield
    
    # Shutdown
    logger.info("Shutting down agent-debugger agent...")
    if agent_instance:
        agent_instance.status = "shutdown"

# Create FastAPI app
app = FastAPI(
    title="Agent Debugger Agent",
    description="SutazAI Agent Debugger Agent API",
    version="1.0.0",
    lifespan=lifespan
)
...
async def root():
    """Root endpoint"""
    return {
        "message": "Agent Debugger Agent API",
...
async def process_task(task: TaskRequest):
    ...
    try:
        ...
        return TaskResponse(
            status="error",
            error=str(e),
            agent=agent_instance.agent_id if agent_instance else "agent-debugger"
        )
