...
class TaskResponse(BaseModel):
    ...
    result: Any
    agent: str = "aider"
    capabilities: List[str] = ['code_editing', 'git_integration', 'pair_programming']

class AgentInfo(BaseModel):
    id: str = "aider"
    name: str = "Aider"
    description: str = "AI pair programming assistant"
    capabilities: List[str] = ['code_editing', 'git_integration', 'pair_programming']
    framework: str = "aider"
    status: str = "active"
...
@app.get("/")
async def root():
    return {"agent": "Aider", "status": "active"}

@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "aider"}
...
async def process_task(request: TaskRequest) -> Dict[str, Any]:
    ...
    if "aider" == "autogpt" and "autonomous" in task:
        return await handle_autonomous_task(request)
    elif "aider" == "crewai" and "team" in task:
        return await handle_team_task(request)
    elif "aider" == "aider" and "code" in task:
...
async def handle_generic_task(request: TaskRequest) -> Dict[str, Any]:
    ...
    return {
        "task": request.task,
        "agent": "aider",
        "result": f"Task processed by Aider"
    }
