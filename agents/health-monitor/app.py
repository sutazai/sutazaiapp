...
app = FastAPI(title="Health Monitor Agent")
...
class TaskResponse(BaseModel):
    status: str
    result: Any
    agent: str = "health-monitor"
    capabilities: List[str] = ['health_checks', 'system_monitoring', 'alert_management']

class AgentInfo(BaseModel):
    id: str = "health-monitor"
    name: str = "Health Monitor"
    description: str = "System health monitoring"
    capabilities: List[str] = ['health_checks', 'system_monitoring', 'alert_management']
...
@app.get("/")
async def root():
    return {"agent": "Health Monitor", "status": "active"}

@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "health-monitor"}
...
async def process_task(request: TaskRequest) -> Dict[str, Any]:
    ...
    if "health-monitor" == "autogpt" and "autonomous" in task:
        return await handle_autonomous_task(request)
    elif "health-monitor" == "crewai" and "team" in task:
        return await handle_team_task(request)
    elif "health-monitor" == "aider" and "code" in task:
...
async def handle_generic_task(request: TaskRequest) -> Dict[str, Any]:
    ...
    return {
        ...
        "result": f"Task processed by Health Monitor"
    }
