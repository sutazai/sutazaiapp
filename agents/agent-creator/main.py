#!/usr/bin/env python3
"""
FastAPI main application for agent-creator agent
...
from app import Agent_CreatorAgent
...
agent_instance: Agent_CreatorAgent = None
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting agent-creator agent...")
    agent_instance = Agent_CreatorAgent()
    logger.info(f"Agent {agent_instance.name} initialized on port {agent_instance.port}")
    
    yield
    
    # Shutdown
    logger.info("Shutting down agent-creator agent...")
    if agent_instance:
...
app = FastAPI(
    ...
    description="SutazAI Agent Creator Agent API",
    version="1.0.0",
    lifespan=lifespan
)
...
async def root():
    """Root endpoint"""
    return {
        "message": "Agent Creator Agent API",
        "agent": "agent-creator",
        "status": "active",
        "version": "1.0.0"
    }
...
async def process_task(task: TaskRequest):
    ...
    try:
        ...
        return TaskResponse(
            status="error",
            error=str(e),
            agent=agent_instance.agent_id if agent_instance else "agent-creator"
        )
