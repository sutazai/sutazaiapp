...
class RequirementsOptimizer:
                ...
                for line in lines:
                    ...
                    try:
                        ...
                        if req.name.lower() not in self.parsed_requirements:
                            package_req = PackageRequirement(name=req.name.lower())
                            self.parsed_requirements[req.name.lower()] = package_req
                        else:
                            package_req = self.parsed_requirements[req.name.lower()]
                        ...
                        print(f"Warning: Could not parse {line} from {req_file}: {e}")
    ...
    def generate_optimized_requirements(self) -> None:
        ...
        if os.path.exists(self.output_file):
            ...
            print(f"Backing up existing requirements.txt to {self.backup_file}")
        ...
        with open(self.output_file, "w", encoding="utf-8") as f:
            ...
            f.write("# This file consolidates all requirements from multiple files\n")
            f.write("\n")
            
            # Sort packages alphabetically
            sorted_packages = sorted(
                self.parsed_requirements.values(), 
                key=lambda x: x.name
            )
