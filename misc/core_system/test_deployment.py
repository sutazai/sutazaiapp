...
class TestDeployment:
    ...
    def test_required_services_ports(self):
        ...
        test_ports = {"postgres": 5432, "redis": 6379, "application": 8000}
        ...
        for service, port in test_ports.items():
            try:
                ...
                assert (
                    result == 0
                ), f"{service.capitalize()} port {port} is not available"
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "deployment: mark test as a deployment validation test"
    )
