...
from .surface_codes import ErrorModel
...
class LatticeSurgeryOperator:
    ...
    def _correct_errors(
        self, noisy_qubit: np.ndarray, syndrome: Dict[str, np.ndarray]
    ) -> np.ndarray:
...
class LatticeSurgerySimulator:
    ...
    def simulate_error_correction(
        self, logical_qubit: np.ndarray, num_iterations: int = 100
    ) -> Dict[str, Any]:
        ...
        results["correction_success_rate"] = (
            results["successful_corrections"] / num_iterations
        )
