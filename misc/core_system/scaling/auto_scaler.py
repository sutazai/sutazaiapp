...
class AutoScaler:
    ...
    def adjust_resources(
        ...
        try:
            ...
            if cpu_usage > self.max_scale_up_threshold:
                if (
                    self.current_resources["cpu_cores"]
                    < self.current_resources["max_cpu_cores"]
                ):
                    self.current_resources["cpu_cores"] *= 2
                    self.current_resources["memory_gb"] *= 2
                    self._log_scaling_event(
                        "scale_up", old_resources, self.current_resources
                    )
            ...
            # Scale-down logic
                ...
                if self.current_resources["cpu_cores"] > 1:
                    ...
                    self.current_resources["memory_gb"] //= 2
                    self._log_scaling_event(
                        "scale_down", old_resources, self.current_resources
                    )
    ...
    def get_scaling_recommendations(self) -> Dict[str, Any]:
        ...
        if len(self.scaling_history) > 1:
            time_between_events = [
                self.scaling_history[i + 1]["timestamp"]
