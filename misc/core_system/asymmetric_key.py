...
class AsymmetricKey(Key):
    ...
    def as_dict(self, is_private=False, **params):
        ...
        if "d" in tokens and not is_private:
            ...
            tokens = {
                k: tokens[k] for k in tokens if k in self.PUBLIC_KEY_FIELDS
            }
            tokens["kty"] = self.kty
            if kid:
                tokens["kid"] = kid
    ...
    def as_der(self, is_private=False, password=None):
        return self.as_bytes(
            encoding="DER", is_private=is_private, password=password
        )
    ...
    def import_key(cls, raw, options=None):
        ...
        if isinstance(raw, cls.PUBLIC_KEY_CLS):
            ...
            if options is not None:
                ...
                password = None
            raw_key = load_pem_key(
                raw, cls.SSH_PUBLIC_PREFIX, password=password
            )
