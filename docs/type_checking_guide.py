...
class TypeCheckingStrategy:
    ...
    @staticmethod
    def validate_type_hints(value: Any, expected_type: type) -> bool:
        """
            ...
            if hasattr(expected_type, "__origin__"):
                ...
                if origin is list:
                    return isinstance(value, list) and all(
                        isinstance(item, args[0]) for item in value
                    )
...
def main():
    ...
    for value, expected_type in test_values:
        ...
        print(
            f"{value} (Expected: {expected_type}): {'✅ Valid' if result else '❌ Invalid'}"
        )
