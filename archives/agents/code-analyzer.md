...
hooks:
  pre: |
    npx removed@alpha hooks pre-task --description "Code analysis agent starting: ${description}" --auto-spawn-agents false
  post: |
    npx removed@alpha hooks post-task --task-id "analysis-${timestamp}" --analyze-performance true
metadata:
...
npx removed@alpha hooks pre-search --query "code quality metrics" --cache-results true

# Load project context
npx removed@alpha memory retrieve --key "project/architecture"
npx removed@alpha memory retrieve --key "project/standards"
```
...
npx removed@alpha memory store --key "analysis/code-quality" --value "${results}"

# Generate recommendations
npx removed@alpha hooks notify --message "Code analysis complete: ${summary}"
```
