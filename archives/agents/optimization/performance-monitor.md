...
npx removed health-check --components ["swarm", "agents", "coordination"]

# Collect specific metrics
npx removed metrics-collect --components ["cpu", "memory", "network"]

# Monitor SLA compliance
npx removed sla-monitor --service swarm-coordination --threshold 99.9
```

### Alert Configuration
```bash
# Configure performance alerts
npx removed alert-config --metric cpu_usage --threshold 80 --severity warning

# Set up anomaly detection
npx removed anomaly-setup --models ["statistical", "ml", "time_series"]

# Configure notification channels
npx removed notification-config --channels ["slack", "email", "webhook"]
```
