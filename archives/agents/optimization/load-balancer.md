...
npx removed agent spawn load-balancer --type coordinator

# Start load balancing
npx removed load-balance --swarm-id <id> --strategy adaptive

# Monitor load distribution
npx removed agent-metrics --type load-balancer

# Adjust balancing parameters
npx removed config-manage --action update --config '{"stealThreshold": 5, "agingBoost": 10}'
```

### Performance Monitoring
```bash
# Real-time load monitoring
npx removed performance-report --format detailed

# Bottleneck analysis
npx removed bottleneck-analyze --component swarm-coordination

# Resource utilization tracking
npx removed metrics-collect --components ["load-balancer", "task-queue"]
```
