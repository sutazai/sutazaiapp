...
gh pr view 123 --json body,title,labels,files | npx removed swarm create-from-pr

# Auto-spawn agents based on PR labels
gh pr view 123 --json labels | npx removed swarm auto-spawn

# Create swarm with PR context
gh pr view 123 --json body,labels,author,assignees | \
  npx removed swarm init --from-pr-data
```
      ...
      - name: Handle Swarm Command
        run: |
          if [[ "${{ github.event.comment.body }}" == /swarm* ]]; then
            npx removed github handle-comment \
...
npx removed github pr-topology --pr 123
```
...
npx removed github pr-init 123 \
  --auto-agents \
  --pr-data "$PR_INFO" \
  --diff "$PR_DIFF" \
  --analyze-impact
...
PROGRESS=$(npx removed github pr-progress 123 --format markdown)
...
# Run swarm review
REVIEW_RESULTS=$(npx removed github pr-review 123 \
...
npx removed github pr-deps 123 \
  --spawn-agents \
  --resolve-conflicts
```
...
SWARM_STATUS=$(npx removed github pr-status 123)
...
createServer((req, res) => {
  if (req.url === '/github-webhook') {
    ...
    if (event.action === 'opened' && event.pull_request) {
      execSync(`npx removed github pr-init ${event.pull_request.number}`);
    }
...
npx removed github pr-init 456 \
...
npx removed github pr-init 789 \
  --topology mesh \
...
npx removed github pr-init 321 \
  --topology ring \
...
npx removed github pr-report 123 \
...
npx removed github export-metrics \
  --pr 123 \
  --to-insights
```
...
mcp__removed__swarm_init { topology: "mesh", maxAgents: 8 }
mcp__removed__agent_spawn { type: "coordinator", name: "PR Coordinator" }
mcp__removed__agent_spawn { type: "reviewer", name: "Code Reviewer" }
mcp__removed__agent_spawn { type: "tester", name: "Test Engineer" }
mcp__removed__agent_spawn { type: "analyst", name: "Impact Analyzer" }
...
mcp__removed__coordination_sync { swarmId: "pr-review-swarm" }
