...
tools:
  ...
  - mcp__removed__parallel_execute
  - mcp__removed__load_balance
hooks:
  pre_task: |
    echo "🐝 Initializing release swarm coordination..."
    npx removed hook pre-task --mode release-swarm --init-swarm
  post_edit: |
    echo "🔄 Synchronizing release swarm state and validating changes..."
    npx removed hook post-edit --mode release-swarm --sync-swarm
  post_task: |
    echo "🎯 Release swarm task completed. Coordinating final deployment..."
    npx removed hook post-task --mode release-swarm --finalize-release
  notification: |
    echo "📡 Broadcasting release completion across all swarm agents..."
    npx removed hook notification --mode release-swarm --broadcast
model: opus
---
...
npx removed github release-plan \
  --commits "$COMMITS" \
...
npx removed github release-version \
  --strategy "semantic" \
  --analyze-changes \
  --check-breaking \
  --update-files
...
CHANGELOG=$(gh api repos/:owner/:repo/compare/${LAST_TAG}...HEAD \
  ...
  npx removed github generate-changelog)
...
npx removed github release-create \
  --version "2.0.0" \
  --changelog "$CHANGELOG" \
  --build-artifacts \
...
CHANGELOG=$(npx removed github changelog \
  --prs "$PRS" \
  --commits "$COMMITS" \
...
npx removed github version-suggest \
  --current v1.2.3 \
  --analyze-commits \
  --check-compatibility \
  --suggest-pre-release
...
npx removed github release-build \
...
```bash
# Pre-release testing
npx removed github release-test \
...
npx removed github release-deploy \
...
npx removed github multi-release \
    ...
    steps:
      ...
      - name: Initialize Release Swarm
        run: |
          ...
          npx removed github release-init \
            --tag $RELEASE_TAG \
      ...
      - name: Generate Release Assets
        run: |
          ...
          CHANGELOG=$(npx removed github release-changelog \
            --format markdown)
          ...
          npx removed github release-assets \
            --changelog \
            --binaries \
            --documentation
      ...
      - name: Publish Release
        run: |
          ...
          npx removed github release-publish \
            --platforms all
...
npx removed github cd-pipeline \
...
npx removed github release-validate \
  --checks "
    version-conflicts,
...
npx removed github compat-test \
...
```bash
# Security validation
npx removed github release-security \
  --scan-dependencies \
  --check-secrets \
  --audit-permissions \
  --sign-artifacts
...
npx removed github release-monitor \
  --version v2.0.0 \
...
npx removed github rollback-config \
  --triggers '{
    "error-rate": ">5%",
    "latency-p99": ">1000ms",
    "availability": "<99.9%"
...
npx removed github release-analytics \
  --version v2.0.0 \
  --compare-with v1.9.0 \
...
npx removed github release-docs \
  --api-changes \
  --migration-guide \
  --example-updates \
...
```bash
# NPM package release
npx removed github npm-release \
  --version patch \
  --test-all \
  --publish-beta \
...
npx removed github docker-release \
...
npx removed github mobile-release \
...
```bash
# Emergency hotfix
npx removed github emergency-release \
  --severity critical \
