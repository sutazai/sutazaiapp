...
tools: mcp__removed__swarm_init, mcp__removed__agent_spawn, mcp__removed__task_orchestrate, Bash, Read, Write, TodoWrite
color: blue
type: development
capabilities:
...
npx removed github review-init \
  --pr 123 \
  --pr-data "$PR_DATA" \
  --diff "$PR_DIFF" \
...
# Run security review
SECURITY_RESULTS=$(npx removed github review-security \
  --pr 123 \
  --files "$CHANGED_FILES" \
...
npx removed github review-performance \
  --pr 123 \
...
```bash
# Architecture review
npx removed github review-architecture \
  --pr 123 \
...
npx removed github review-context \
  --pr 123 \
  --load-related-prs \
  --analyze-impact \
...
npx removed github review-learn \
...
npx removed github review-batch \
  --prs "123,124,125" \
  --check-consistency \
  --verify-integration \
  --combined-impact
    ...
    steps:
      ...
      - name: Run Review Swarm
        run: |
          ...
          # Run swarm review
          REVIEW_OUTPUT=$(npx removed github review-all \
            --pr $PR_NUM \
            --pr-data "$PR_DATA" \
...
COMMENTS=$(npx removed github review-comment \
  --pr 123 \
  --diff "$PR_DIFF" \
  --files "$PR_FILES" \
  --style "constructive" \
...
npx removed github review-comments \
  --pr 123 \
...
npx removed github quality-gates \
  --define '{
...
npx removed github review-metrics \
  --period 30d \
...
npx removed github review-train \
  --learn-patterns \
  --adapt-to-style \
  --improve-accuracy
```
...
npx removed github review-orchestrate \
  --strategy "risk-based" \
...
npx removed github review-dashboard \
  --real-time \
...
npx removed github review-report \
  --format "markdown" \
