...
tools:
  ...
  - mcp__removed__github_sync_coord
...
npx removed github multi-repo-init \
...
npx removed github discover-repos \
  --repos "$REPOS" \
  --dependencies "$DEPS" \
...
echo "$MATCHING_REPOS" | while read -r repo; do
  ...
  # Execute task
  cd /tmp/$repo
  npx removed github task-execute \
...
npx removed github link-prs --urls "$PR_URLS"
```
...
npx removed github multi-repo-refactor \
...
npx removed github multi-repo-security \
  --scan-all \
...
const { MultiRepoSwarm } = require('removed');
...
npx removed github multi-repo-queue \
  --backend redis \
  --workers 10 \
  --priority-routing \
  --dead-letter-queue
...
npx removed github multi-repo-test \
  --setup-test-env \
  --link-services \
  --run-e2e \
  --tear-down
...
npx removed github to-monorepo \
  --analyze-repos \
  --suggest-structure \
  --preserve-history \
...
npx removed github multi-repo-dashboard \
  --port 3000 \
...
npx removed github health-check \
  --repos "org/*" \
...
npx removed github microservices \
...
npx removed github cache-strategy \
  --analyze-patterns \
...
npx removed github parallel-optimize \
  --analyze-dependencies \
...
npx removed github resource-pool \
  --share-agents \
  --distribute-load \
  --monitor-usage
```
...
npx removed github diagnose-connectivity \
  --test-all-repos \
  --check-permissions \
  --verify-webhooks
```
...
npx removed github debug-memory \
  --check-consistency \
  --identify-conflicts \
  --repair-state
```
...
npx removed github perf-analysis \
  --profile-operations \
...
npx removed github fullstack-update \
