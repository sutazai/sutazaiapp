...
SERVICE_NAME="backend-agi"  # Replace with failing service
...
docker-compose restart ollama backend-agi
...
docker exec sutazai-backend-agi curl -s -o /dev/null -w "%{http_code}" http://postgres:5432 || echo "Backend->Postgres: FAIL"
docker exec sutazai-backend-agi curl -s -o /dev/null -w "%{http_code}" http://redis:6379 || echo "Backend->Redis: FAIL"
docker exec sutazai-backend-agi curl -s -o /dev/null -w "%{http_code}" http://ollama:11434 || echo "Backend->Ollama: FAIL"
...
docker-compose stop backend-agi frontend-agi
