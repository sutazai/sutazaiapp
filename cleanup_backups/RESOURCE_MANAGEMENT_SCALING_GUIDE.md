...
```yaml
# High Priority Services (Brain & Core Infrastructure)
ollama:
  deploy:
    resources:
      limits:
...
Application Layer:
  backend-agi: 4GB (FastAPI + AI orchestration)
  frontend-agi: 2GB (Streamlit interface)
...
upstream backend_cluster {
    least_conn;
    server backend-agi_1:8000 max_fails=3 fail_timeout=30s;
    server backend-agi_2:8000 max_fails=3 fail_timeout=30s;
    server backend-agi_3:8000 max_fails=3 fail_timeout=30s;
...
CURRENT_LOAD=$(docker stats --no-stream --format "{{.CPUPerc}}" sutazai-backend-agi | sed 's/%//')
if (( $(echo "$CURRENT_LOAD > 80" | bc -l) )); then
    scale_up_service "backend-agi" "4" "8G"
fi
...
main() {
    ...
    services=(
        "sutazai_backend-agi:sutazai-backend-agi"
