...
class ValidationError(Exception):    """Custom exception for validation failures."""    class SystemValidator:        """Comprehensive system validation framework."""        def __init__(self):            """Initialize comprehensive system validation framework."""            self.critical_dirs = [                "/opt/sutazaiapp/ai_agents",                "/opt/sutazaiapp/model_management",                "/opt/sutazaiapp/backend",                "/opt/sutazaiapp/scripts",            ]            self.required_models = ["gpt4all", "deepseek-coder", "llama2"]
    self.console = Console()
    def validate_system_requirements(self) -> None:                """                Comprehensive system requirements validation.                Raises:                ValidationError: If any system requirement is not met                """                logger.info("🔍 Starting Comprehensive System Validation")                # Python version check                logger.info("Python Version: %s", sys.version)                if not (sys.version_info >= (3, 11)):                    raise ValidationError("Python 3.11+ is required")
        ...
        def _validate_os_and_hardware(self) -> None:                        """                        Validate operating system and hardware specifications.                        Raises:                        ValidationError: If hardware requirements are not met                        """                        logger.info("Checking OS and Hardware Configuration")                        # CPU Information                        cpu_count = psutil.cpu_count(logical=False)                        logger.info("Physical CPU Cores: %s", cpu_count)
            if cpu_count < 8:                            raise ValidationError(
                ...
                if total_memory < 32:                                raise ValidationError(
                    ...
                    def _validate_critical_directories(                                    self) -> None:                                    """                                    Validate existence and permissions of critical directories.                                    Raises:                                    ValidationError: If any critical directory is missing                                    """                                    logger.info(                                        "Checking Critical Directories")                                    missing_dirs = []
                        for directory in self.critical_dirs:                                            if not os.path.exists(directory):                                            missing_dirs.append(directory)
                            ...
                            if missing_dirs:                                                raise ValidationError(
                                ...
                                def _validate_network_config(                                                    self) -> None:                                                    """                                                    Validate network configuration and connectivity.                                                    Raises:                                                    ValidationError: If network configuration is invalid                                                    """                                                    logger.info(                                                        "Checking Network Configuration")                                                    try:                                                            # Test network connectivity using full path and safe subprocess
                                    # execution
                                        ...
                                        if not os.path.exists(
                                            ...
                                            "Network connectivity: OK")
                                                ...
                                                f"Network connectivity test failed: {e}") from e
                                                    ...
                                                    def validate_model_availability(                                                                            self) -> None:                                                                            """                                                                            Check AI model availability and basic loading.                                                                            Raises:                                                                            ValidationError: If required models are not available                                                                            """                                                                            logger.info(                                                                                "🤖 Validating AI Model Availability")                                                                            missing_models = []
                                                        ...
                                                        if not os.path.exists(
                                                            ...
                                                            model_name)
                                                                ...
                                                                if missing_models:                                                                                        raise ValidationError(
                                                                    ...
                                                                    def run_comprehensive_tests(                                                                                            self) -> None:                                                                                            """                                                                                            Execute comprehensive system tests.                                                                                            Raises:                                                                                            ValidationError: If any validation check fails                                                                                            SystemExit: If pytest detects test failures                                                                                            """                                                                                            try:                                                                                                self.validate_system_requirements()                                                                                                self.validate_model_availability()
                                                                        # Run
                                                                        # pytest
                                                                        # for
                                                                        # additional
                                                                        ...
                                                                        if pytest_result != 0:                                                                                                    logger.error(
                                                                            ...
                                                                            "🎉 Comprehensive System Validation Complete!")
                                                                                ...
                                                                                1)
                                                                                    ...
                                                                                    sys.exit(
                                                                                    1)
                                                                                    def generate_test_report(                                                                                                                self) -> Dict[str, Any]:                                                                                                                """                                                                                                                Generate a comprehensive test report.                                                                                                                Returns:                                                                                                                Dict[str, Any]: Test report data                                                                                                                """                                                                                                                report = {                                                                                                                    "timestamp": datetime.now().isoformat(),                                                                                                                    "python_version": sys.version,
                                                                                        "system_info": {
