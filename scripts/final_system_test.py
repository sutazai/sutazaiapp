...
This script performs a comprehensive system test and \
    audit of the SutazAI application.
"""
...
def run_python_script(script_path: str) -> Tuple[bool, str]:
    """
    Run a Python script and return whether it succeeded and its output.
    ...
    def main() -> None:
        """Main execution function for final system test"""
        console = Console()
        console.rule(
        ...
        for task_name, script_path in tasks:
            ...
            console.print(
                Panel(output, title=f"{task_name} Output", expand=False))
            ...
            try:
                ...
                output = result.stdout
                    ...
                    summary["Unit Tests"] = {"success": str(
                        success), "output": output}
                    status = "[green]Success[/green]" if success else "[red]Failed[/red]"
                    console.print(f"Unit Tests: {status}")
                    console.print(
                        Panel(output, title="Unit Tests Output", expand=False))
