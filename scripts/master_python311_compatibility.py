...
def verify_python_version() -> None:
    ...
    if major != 3 or minor != 11:
        raise RuntimeError("This script requires Python 3.11. " f"Current version: {sys.version}")
...
def generate_compatibility_report(
    ...
    with open(report_path, "w") as report_file:
        ...
        report_file.write(f"**Generated:** {os.getenv('TIMESTAMP', 'Unknown')}\n\n")

        report_file.write("## Script Execution Summary\n\n")

        success_count = sum(1 for result in script_results if result["status"] == "success")
        failed_count = sum(1 for result in script_results if result["status"] == "failed")
        skipped_count = sum(1 for result in script_results if result["status"] == "skipped")
        ...
        for result in script_results:
            ...
            if result["status"] == "success":
                ...
                report_file.write(f"- **Reason:** {result.get('reason', 'No reason')}\n")
