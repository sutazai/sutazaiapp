...
This script checks the project root and subdirectories for files and folders
that are not part of the expected structure. Any extraneous files found in
the root will be moved to a 'misc' directory for further inspection.
Expected structure at project root:Directories: ai_agents, model_management, backend, web_ui, scripts, packages, logs, doc_data, venv
Files: README.md, requirements.txt
Usage:python3 scripts/organize_project.py
...
class ProjectOrganizer:    """Project organization utility."""    def __init__(self, project_root: str = "/opt/sutazaiapp"):        """        Initialize the project organizer.        Args:        project_root: Root directory of the project        """        self.project_root = Path(project_root)        self.misc_dir = self.project_root / "misc"        self.expected_dirs: Set[str] = {            "ai_agents",
    "model_management",
    "backend",
    "web_ui",
    "scripts",
    ...
    def should_ignore(self, path: Path) -> bool:            """            Check if a path should be ignored.            Args:            path: Path to check            Returns:            bool: True if path should be ignored            """            return any(            pattern in path.name or path.match(pattern)            for pattern in self.ignored_patterns
        ...
        def organize_root(self) -> Dict[str, List[str]]:            """                Organize the project root directory.                Returns:                Dict[str, List[str]]: Report of moved files and errors                """            report = {                "moved_files": [],                "errors": [],            }
            logger.info("Starting organization of project root...")
                        ...
                        if dest.exists():                                # Add timestamp to avoid conflicts
                            ...
                            report["moved_files"].append(str(item))
                                ...
                                report["errors"].append(error_msg)
                                    ...
                                    report["errors"].append(error_msg)
                                    return report
                                def verify_structure(self) -> Dict[str, List[str]]:                                    """                                        Verify the project structure.                                        Returns:                                        Dict[str, List[str]]: Report of missing items                                        """                                    report = {                                        "missing_dirs": [],                                        "missing_files": [],                                    }
