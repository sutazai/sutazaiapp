...
This script checks the project root and \
    subdirectories for files and \
    folders that are not part of the expected structure.
Any extraneous files found in \
        ...
        if not os.path.exists(MISC_DIR):
            ...
            for item in items:
                # Skip expected directories and files
                                if item in EXPECTED_DIRECTORIES or \
                    item in EXPECTED_FILES or \
                    item == MISC_DIR:
                continue
                ...
                try:
                    ...
                    logging.info(f"Moved '{item}' to '{dest}'")
                        ...
                        logging.exception(
                            f"Error moving {item} to {MISC_DIR}: {e}")
