    ...
    def analyze_performance_metrics(self):
        ...
        insights = {
            'cpu_usage': {
                ...
                'avg': sum(metric['percent'] for metric in self.metrics['cpu_usage']) / len(self.metrics['cpu_usage'])
            },
            'memory_usage': {
                'max_percent': max(metric['percent'] for metric in self.metrics['memory_usage']),
                'avg_percent': sum(metric['percent'] for metric in self.metrics['memory_usage']) / len(self.metrics['memory_usage'])
            }
        }
