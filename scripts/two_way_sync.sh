...
optimize_rsync() {
    # Use a more efficient way to check CPU load
    CPU_LOAD=$(awk '{u=$2+$4; t=$2+$4+$5; if (NR==1){u1=u; t1=t;} else print ($2+$4)*100/(t-t1);}' \
        <(grep 'cpu ' /proc/stat) <(sleep 0.1 && grep 'cpu ' /proc/stat))
    
    # Always use nice for rsync operations
    RSYNC_CMD="nice -n 19 ionice -c2 -n7 $RSYNC_CMD"
    
    # Always limit bandwidth to reduce system load
    ...
    # Add additional throttling when system is under heavy load
    if (( $(echo "$CPU_LOAD > 50" | bc -l 2>/dev/null) )); then
        # Reduce chunk size for lower memory usage
        RSYNC_CMD="$RSYNC_CMD --block-size=8192"
        
        # Reduce rsync's priority even further
        RSYNC_CMD="nice -n 19 ionice -c3 $RSYNC_CMD"
        
        # Further reduce bandwidth
        local reduced_bandwidth=$((MAX_BANDWIDTH / 2))
        RSYNC_CMD="$RSYNC_CMD --bwlimit=$reduced_bandwidth"
        
        echo "[$CURRENT_DATE] High CPU load detected ($CPU_LOAD%), applying additional throttling"
    fi
}
