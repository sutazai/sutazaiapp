    ...
    async def _fix_docker_issues(self, issues: List[Issue]):
        ...
        for issue in issues:
            ...
            if "backend" in container_name:
                ...
                result = subprocess.run(
                    ["docker-compose", "-f", "docker-compose-complete-agi.yml", "up", "--build", "-d", "backend-agi"],
...
class Settings(BaseSettings):
    APP_NAME: str = "SutazAI AGI/ASI System"
    API_V1_STR: str = "/api/v1"
            ...
            if "docker-compose" in issue.file_path:
                ...
                if compose_file.exists():
                    try:
                        ...
                        if 'services' in compose_data and 'backend-agi' in compose_data['services']:
                            backend = compose_data['services']['backend-agi']
                        ...
                        self.fixes_applied.append(FixResult(
                            success=True,
                            issue=issue,
                            action_taken="Fixed docker-compose.yml command for backend-agi",
