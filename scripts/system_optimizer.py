#!/opt/sutazaiapp/venv/bin/python3import gcimport loggingimport osimport shutilimport sysimport tempfileimport timeimport importlibfrom pathlib import Pathfrom typing import Any, Dict, List, Optional, Setimport psutil# Configure logginglogging.basicConfig(level=logging.INFO,format="%(asctime)s - %(levelname)s: %(message)s",handlers=[logging.StreamHandler(),logging.FileHandler("/opt/sutazaiapp/logs/system_optimizer.log"),],)logger = logging.getLogger(__name__)class SystemOptimizer:    def __init__(self, project_root: str):        self.project_root = Path(project_root)
self.thresholds = {
"cpu_warning": 70.0,
"cpu_critical": 85.0,
"memory_warning": 75.0,
