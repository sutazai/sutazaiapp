...
if __name__ == "__main__":
    ...
    logger.error(f"Failed: {report['test_summary']['failed_tests']}")
    logger.info(f"Success Rate: {report['test_summary']['success_rate']:.2f}%")
    logger.info(f"Endpoints Tested: {report['test_summary']['unique_endpoints_tested']}")
    logger.info(f"Average Response Time: {report['test_summary']['average_response_time']:.3f}s")
    logger.info(f"Test Duration: {report['total_test_duration']:.2f}s")
