...
try:
    ...
    print("EXPECTED: Permission denied handled")
    exit(0)
except Exception as e:
    print(f"UNEXPECTED ERROR: {{e}}")
    exit(2)
...
if free_mb < 100:  # Less than 100MB
    print("LOW_SPACE: Insufficient disk space detected")
    exit(1)
else:
    print("SUFFICIENT_SPACE: Disk space OK")
    exit(0)
'''
...
def timeout_handler(signum, frame):
    print("TIMEOUT_HANDLED: Process interrupted by timeout")
    exit(0)
...
try:
    ...
    print("UNEXPECTED: Should have been interrupted")
    exit(1)
except KeyboardInterrupt:
    print("KEYBOARD_INTERRUPT: Process interrupted")
    exit(0)
'''
...
try:
    with open(config_file) as f:
        config = json.load(f)
    print("UNEXPECTED: Should have failed to parse JSON")
    exit(1)
except json.JSONDecodeError as e:
    print(f"JSON_ERROR_HANDLED: {{e}}")
    exit(0)
except Exception as e:
...
def test_network_call():
    try:
        ...
        print(f"NETWORK_ERROR_HANDLED: {type(e).__name__}")
        return True
    except Exception as e:
        print(f"UNEXPECTED_ERROR: {e}")
        return False
...
if original_file.exists() and "CORRUPTED" in original_file.read_text():
    print("CORRUPTION_DETECTED: Original file is corrupted")
    
    # Recover from backup
    if backup_file.exists():
        shutil.copy2(backup_file, original_file)
        print("RECOVERY_SUCCESS: File recovered from backup")
        exit(0)
    else:
        print("RECOVERY_FAILED: No backup available")
        exit(1)
else:
    print("NO_CORRUPTION: File is OK")
    exit(0)
'''
...
def safe_operation():
    ...
    try:
        ...
        print(f"PRIMARY_SUCCESS: {result}")
        return result
    except Exception as e:
        print(f"PRIMARY_FAILED: {e}")
        try:
            result = fallback_function()
            print(f"FALLBACK_SUCCESS: {result}")
            return result
        except Exception as e:
...
try:
    with open(state_file) as f:
        state = json.load(f)
    print("STATE_LOAD_SUCCESS")
except json.JSONDecodeError:
    print("STATE_CORRUPTION_DETECTED")
    if restore_state():
        # Try loading again
        with open(state_file) as f:
            state = json.load(f)
        print("STATE_RESTORATION_SUCCESS")
    else:
        print("STATE_RESTORATION_FAILED")
        exit(1)
    ...
    try:
        ...
        for i in range(1000):
            ...
            if i > 500:  # Simulated limit
                print("MEMORY_LIMIT_REACHED: Stopping operation")
                break
                
        print(f"MEMORY_OPERATION_COMPLETE: Allocated {len(data)} chunks")
        
    except MemoryError:
        print("MEMORY_ERROR_HANDLED: Out of memory")
    finally:
        # Cleanup
        data.clear()
        gc.collect()
        print("MEMORY_CLEANUP_COMPLETE")
...
def timeout_handler(signum, frame):
    print("CPU_TIMEOUT_HANDLED: Operation interrupted")
    exit(0)
...
try:
    ...
    while True:
        count += 1
        if count % 1000000 == 0:
            print(f"CPU_WORK_PROGRESS: {count}")
        # This should be interrupted by timeout
        
except KeyboardInterrupt:
    print("CPU_OPERATION_INTERRUPTED")
    exit(0)
'''
...
try:
    ...
    for i in range(100):  # Reasonable number for test
        ...
        if i % 10 == 0:
            print(f"FD_PROGRESS: Opened {{len(opened_files)}} files")
            
    print(f"FD_SUCCESS: Opened {{len(opened_files)}} files total")
    
except OSError as e:
    print(f"FD_LIMIT_REACHED: {{e}}")
...
print(f"CONCURRENT_TEST_COMPLETE: {{len(results)}} workers finished")
exit(0)
'''
