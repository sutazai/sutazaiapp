#!/usr/bin/env python3
"""Update all agent Ollama configurations to use gpt-oss model."""
...
def update_agent_configs():
    """Update all agent _ollama.json files to use gpt-oss."""
    ...
    for config_path in ollama_configs:
        try:
            ...
            # Update the modelfile to use gpt-oss
            if 'modelfile' in config:
                # Replace any FROM model line with gpt-oss
                lines = config['modelfile'].split('\n')
                for i, line in enumerate(lines):
                    if line.startswith('FROM '):
                        lines[i] = 'FROM gpt-oss:latest'
                        break
                config['modelfile'] = '\n'.join(lines)
            ...
            config['max_context_length'] = 2048  # Reduced for gpt-oss
            config['temperature'] = 0.7
            config['max_tokens'] = 1024  # Reduced for efficiency
            
            # Update config section if it exists
            if 'config' in config:
                config['config']['num_predict'] = 2048  # Reduced for gpt-oss
    ...
    print(f"\n✅ Successfully updated {updated_count} agent configurations to use gpt-oss")
