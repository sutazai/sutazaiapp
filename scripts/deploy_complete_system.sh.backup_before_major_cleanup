...
smart_docker_cleanup_and_restart() {
    ...
    # Use the consolidated smart_docker_restart which handles cleanup
    smart_docker_restart
}
...
restart_docker_with_advanced_recovery() {
    ...
    if [ "$is_wsl2" = "true" ]; then
        ...
        # Only restart service, don't kill processes aggressively
        log_info "   → Restarting Docker service gracefully..."
...
setup_docker_environment() {
    ...
    # Simple check - just verify Docker is running
...
restart_docker_with_recovery() {
    ...
    # Only clean up if Docker is not responding
    if ! timeout 3 docker info >/dev/null 2>&1; then
        log_info "   → Docker not responding, gentle restart..."
        systemctl restart docker.service >/dev/null 2>&1 || true
        sleep 5
    fi
...
intelligent_docker_startup() {
    ...
    # Simple restart if not working
    log_info "   → Docker not responding, restarting service..."
    systemctl restart docker.service >/dev/null 2>&1 || true
    sleep 5
...
monitor_docker_health() {
    # Simple Docker health check - just test if Docker responds
