    ...
    def _analyze_file_complexity(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            with open(file_path, encoding="utf-8") as f:
                ...
                complexity["function_count"] = sum(1 for line in lines if line.strip().startswith("def "))
                complexity["class_count"] = sum(1 for line in lines if line.strip().startswith("class "))
                complexity["import_count"] = sum(1 for line in lines if line.strip().startswith(("import ", "from ")))
            ...
            for file in files:
                if file.endswith(".py"):
                    ...
                    dependency_analysis["import_graph"] = {
                        node: list(self.dependency_graph.neighbors(node)) for node in self.dependency_graph.nodes()
                    }
                    ...
                    dependency_analysis["isolated_modules"] = [
                        node for node in self.dependency_graph.nodes() if self.dependency_graph.degree(node) == 0
                    ]
