...
class CompactMonitor:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                for line in result.stdout.strip().split('\n'):
                    if '|' in line and 'sutazai' in line:  # Only show SutazAI containers
                        parts = line.split('|')
                        if len(parts) == 3:
                            name = parts[0].replace('sutazai-', '').replace('-minimal', '')[:15]
                            status = 'Up' in parts[1]
                            health = '(healthy)' if 'healthy' in parts[1] else ''
                            image = parts[2].split(':')[0].split('/')[-1][:15]
                            ...
                            containers.append(f"{name:<15} {icon} {health}")
    ...
    def get_ollama_models(self):
        ...
        try:
            ...
            container_names = ['sutazai-ollama-minimal', 'sutazai-ollama']
                    ...
                    if len(lines) > 1:
                        ...
                        for line in lines[1:]:
                            ...
                            if parts:
                                ...
                                models.append(f"  • {model_name} ({size})")
                        
                        return models[:3] if models else ["  No models loaded"]
                    break
    ...
    def run(self):
        ...
        try:
            ...
            while True:
                ...
                # Line 20: API Status
                print(f"{self.move_to(20)}🌐 API Status: {api_status}{self.clear_line()}", end='')
