    ...
    def __init__(
        ...
        def compress_file(self, file_path: Path) -> None:
            ...
            try:
                ...
                logger.info("Compressed %s to {compressed_path}", file_path)
                except Exception:
                    logger.exception(
                        "Failed to compress {file_path}: {str(e)}")
                    ...
                    def remove_old_logs(self) -> None:
                        ...
                        for file in self.log_dir.glob("**/*.log.gz"):
                            try:
                                if file.stat(
                                    ).st_mtime < cutoff_date.timestamp():
                                    file.unlink()
                                    logger.info(
                                        "Removed old log file: %s",
                                        file)
                                    except Exception:
                                        logger.exception(
                                                    ...
                                                    try:
                                                        ...
                                                        if log_file.stat().st_size > self.max_size:
                                                            self.compress_file(
                                                                ...
                                                                logger.exception(
                                                                    "Failed to process {log_file}: {str(e)}")
                                                                ...
                                                                self.remove_old_logs()
                                                                    ...
                                                                    def main():
                                                                        try:
                                                                            ...
                                                                            rotator.rotate_logs()
                                                                            logger.info(
                                                                                f"Log rotation completed successfully")
                                                                            except Exception:
                                                                                logger.exception(
                                                                                    "Log rotation script failed: {str(e)}")
