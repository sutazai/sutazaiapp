...
intelligent_docker_startup() {
    ...
    if [ "$is_ubuntu_2404" = "true" ] || command -v containerd >/dev/null 2>&1; then
        ...
        if ! systemctl is-active --quiet containerd; then
            log_info "   → Starting containerd (currently stopped)..."
            systemctl start containerd.service >/dev/null 2>&1 || true
            sleep 2
        else
            log_info "   → containerd already running, continuing..."
        fi
    fi
    ...
    if [ "$is_wsl2" = "true" ]; then
        ...
        # Kill any existing dockerd processes first
        pkill -f dockerd >/dev/null 2>&1 || true
        sleep 2
        
        # Start dockerd with better logging
        ...
        if timeout 3 docker version >/dev/null 2>&1; then
            ...
            log_info "   → Direct dockerd failed, cleaning up..."
            kill $dockerd_pid >/dev/null 2>&1 || true
        fi
    else
    ...
    # Phase 6: Final verification attempts (optimized)
    ...
    while [ $attempts -lt 5 ]; do
        ...
        attempts=$((attempts + 1))
        log_info "   → Waiting for Docker... attempt $attempts/5"
        sleep 1
    done
