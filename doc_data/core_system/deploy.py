...
try:    from system_setup import SystemSetup
    except ImportError:        # If we can't import directly, adjust Python path
        ...
        def check_python_version() -> bool:    """        Check if we're running on Python 3.11.        Returns:    bool: True if Python 3.11, False otherwise        """        major, minor = sys.version_info.major, sys.version_info.minor        if major != 3 or minor != 11:        logger.error(            "Python 3.11 is required. Current version: %s.{minor}",            major)
            return False
        return True
    def deploy_services(base_path: str = "/opt/sutazaiapp") -> None:    """        Deploy all system services.        Args:    base_path: The base path of the application        """        logger.info(f"Deploying system services...")        log_dir = os.path.join(base_path, "logs")        os.makedirs(log_dir, exist_ok=True)        for name, cmd in SERVICES:        try:        # Convert command string to list for security
        cmd_parts = cmd.split()
        ...
        time.sleep(2)
            ...
            raise
            def rollback_deployment() -> None:    """        Rollback deployment in case of failure.        """        logger.info(        f"Rolling back deployment due to errors...")        # Kill deployed services        try:        subprocess.run(            ["pkill", "-f", "uvicorn"],
                check=False)
                subprocess.run(
                ["pkill", "-f", "streamlit"],
                check=False)
                logger.info(f"Services terminated")
                    ...
                    def main() -> None:    """        Main deployment function.        """        try:        logger.info(            f"Starting SutazAI deployment...")        # Check Python version        if not check_python_version():        logger.warning(            f"Continuing deployment despite Python version mismatch")
