...
app = FastAPI(
    title=AGENT_NAME,
    version="2.2.0",
    description=f"{AGENT_ROLE} - Performs advanced data analysis and engineering tasks"
)
    ...
    return {
        ...
        "capabilities": {
            "data_analysis": {
                ...
                "methods": ["exploratory_analysis", "correlation_analysis", "trend_detection"]
            },
            "etl_pipelines": {
                "description": "Build and manage ETL pipelines",
                "methods": ["data_extraction", "data_transformation", "data_loading"]
            },
            "statistical_modeling": {
                "description": "Create statistical models",
                "methods": ["regression_analysis", "time_series", "hypothesis_testing"]
            },
            "data_visualization": {
...
async def execute_task(task: TaskRequest):
    task_id = f"data_{int(time.time() * 1000)}"
    ...
    if redis_client:
        try:
            ...
            # Add to task queue
            redis_client.lpush("task_queue:data_analysis", task_id)
        except Exception as e:
            logger.error(f"Failed to store task in Redis: {e}")
    
    # Simulate task processing time based on type
    estimated_completion = 30
    if task.task_type == "exploratory_analysis":
        estimated_completion = 60
    elif task.task_type == "build_etl_pipeline":
